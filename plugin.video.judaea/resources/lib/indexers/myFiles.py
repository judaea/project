...
from resources.lib.modules import control, source_utils
from resources.lib.modules import premiumize, real_debrid
...
class Menus:
    ...
    def home(self):
        ...
        for file in self.real_debrid_files:
            ...
            control.addDirectoryItem(file['filename'], action, isPlayable=isPlayable, isFolder=isFolder,
        ...
        for file in self.premiumize_files:
            ...
            control.addDirectoryItem(file['name'], action, isPlayable=isPlayable, isFolder=isFolder,
                                   actionArgs=actionArgs)
    ...
    def myFilesPlay(self, args):
        ...
        item = control.menuItem(path=resolved_link)
    ...
    def get_real_debrid_folder(self, args):
        ...
        for file in selected_files:
            ...
            if name.startswith('/'):
                name = name.split('/')[-1]
            control.addDirectoryItem(name, 'myFilesPlay', isPlayable=True, isFolder=False,
                                   actionArgs=args)

        control.closeDirectory('video')
