...
def __get_cache_core(query):
    if __cache_results.get(query, '') is not '':
        if CACHE_LOG:
            control.log('get_cache_local', 'notice')
        return __cache_results[query]
    else:
        __cache_results[query] = {}

    if CACHE_LOG:
        control.log('get_cache_request', 'notice')

    response = __dynamo_get(__map_in_cache(query))

    if response.status_code != 200:
        if CACHE_LOG:
            control.log('get_cache_err_response', 'notice')
        return __cache_results[query]

    result = __map_out_cache(response.text)
    if result is None:
        if CACHE_LOG:
            control.log('get_cache_nocache', 'notice')
        return __cache_results[query]
    ...
    if CACHE_LOG:
        control.log('get_cache_result', 'notice')
...
def __set_cache_core(scraper, query, results, cached_results):
    ...
    try:
        if not __cache_has_new_results[query]:
            if CACHE_LOG:
                control.log('set_cache_skip_no_new_results', 'notice')
            return
        ...
        if CACHE_LOG:
            control.log('set_cache_request', 'notice')
