...
class Premiumize:
    ...
    def auth(self):
        ...
        control.progressDialog.create(control.addonName,
                                    line1=control.lang(32024).format(control.colorString(token['verification_uri'])),
                                    line2=control.lang(32025).format(control.colorString(token['user_code'])))
        ...
        if success:
            control.showDialog.ok(control.addonName, control.lang(32026))
    ...
    def get_url(self, url):
        if self.headers['Authorization'] == 'Bearer ':
            control.log('User is not authorised to make PM requests')
            return None
    ...
    def _single_magnet_resolve(self, magnet, args, pack_select=False):
        ...
        if control.getSetting('premiumize.transcoded') == 'true':
            if selectedFile['transcode_status'] == 'finished':
                try:
                    if selectedFile['stream_link'] is not None and control.getSetting('premiumize.addToCloud') == 'true':
                ...
                pass
        try:
            if selectedFile['link'] is not None and control.getSetting('premiumize.addToCloud') == 'true':
                transfer = self.create_transfer(magnet)
                database.add_premiumize_transfer(transfer['id'])
        except:
            pass
    ...
    def resolve_magnet(self, magnet, args, torrent, pack_select):
        ...
        try:
            if streamLink is not None and control.getSetting('premiumize.addToCloud') == 'true':
                transfer = self.create_transfer(magnet)
                database.add_premiumize_transfer(transfer['id'])
        except:
            pass
    ...
    def user_select(self, content):
        ...
        selection = control.showDialog.select('{}: {}'.format(control.addonName, control.lang(40297)),
