...
class providers:
    ...
    def uninstall_package(self, package=None, silent=False):
        ...
        if package is None:
            if len(packages) == 0:
                control.showDialog.ok(control.addonName, control.lang(32074))
                return
            selection = control.showDialog.select("%s: %s %s" %
                                                (control.addonName, control.lang(32075), control.lang(40068)), packages)
            if selection == -1:
                return
            package_name = packages[selection]
            confirm = control.showDialog.yesno(control.addonName, control.lang(32076) + " %s" % package_name)
            if confirm == 0:
                return
        else:
        ...
        try:
            ...
            time.sleep(2)
            if not silent:
                control.showDialog.ok(control.addonName, '%s ' % package_name + control.lang(32077))
        except:
            control.showDialog.ok(control.addonName,
                                control.language(40118).enocde('utf-8') % (control.addonName, package_name))
    ...
    def install_package(self, install_style, url=None):
        ...
        if url is None:
            if install_style is None:
                install_style = control.showDialog.select(control.addonName, [control.lang(40302), control.lang(40303)])
            else:
    ...
    def install_zip(self, zip_file, silent=False):
        ...
        if remote_meta == '':
            control.showDialog.ok(control.addonName, control.lang(33016))

        line1 = control.colorString(control.lang(33001)) + " %s - v%s" % (pack_name, version)
        line2 = control.colorString(control.lang(33002)) + "%s" % author
        line3 = control.lang(33003)

        if not silent:
            accept = control.showDialog.yesno(control.addonName + " - %s" % control.lang(33004),
                                            line1, line2, line3, control.lang(33005),
                                            control.lang(33006))
            if accept == 0:
                return
        ...
        if os.path.isfile(meta_output_location):
            ...
            try:
                ...
                control.log('Failed to create temporary meta file')
                if not silent:
                    control.showDialog.ok(control.addonName, control.lang(33007))
                return
            ...
            try:
                ...
                self.failure_cleanup(meta_output_location, pack_name, folders)
                control.log('Failed to create new meta file')
                if not silent:
                    self.failed_prompt()
                return
        ...
        if not silent:
            ...
            install_progress.create(control.addonName, '%s - %s' % (pack_name, control.lang(33008)),
                                    control.lang(33009))
        ...
        try:
            for folder in folders:
                try:
                    ...
                    control.log('Failed to extract to folder - %s' % folder)
                    import traceback
                    traceback.print_exc()
                    self.failure_cleanup(meta_output_location, pack_name, folders)
                    if not silent:
            ...
            if not silent:
                try:
                    ...
                    pass
            if not silent:
                control.showDialog.ok(control.addonName, '%s - %s' % (control.lang(33010), pack_name))
        except:
            import traceback
            traceback.print_exc()
            if not silent:
                try:
                    install_progress.close()
                    control.showDialog.ok(control.addonName, '%s - %s' % (control.lang(33012), pack_name),
                                        control.lang(33011))
                except:
                    pass
            return
        ...
        control.log('Refreshing provider database ')
        database.add_provider_package(pack_name, author, remote_meta, version)

        control.execute('RunPlugin("plugin://plugin.video.%s/?action=refreshProviders")' % control.addonName.lower())
        return True

    def failed_prompt(self):
        control.showDialog.ok(control.addonName, control.lang(33013), control.lang(33011))
    ...
    def output_meta(self, meta):
        try:
            ...
            control.log('Failed to create new meta file for provider')
            import traceback
            traceback.print_exc()
            raise Exception
    ...
    def check_for_updates(self, silent=False, automatic=False):
        ...
        if not silent:
            update_dialog = control.progressDialog
            update_dialog.create(control.addonName, control.lang(33019))
        ...
        for package in packages:
            ...
            if not meta_file['name'] == package['pack_name']:
                control.log('Pack name check failure')
                continue
    ...
    def update(self, meta_file, silent=False):
        ...
        if result is not None:
            control.showDialog.notification(control.addonName, control.lang(33017) % package_name)

    def check_version_numbers(self, current, new):
        return control.check_version_numbers(current, new)
    ...
    def manual_update(self):
        ...
        selection = control.showDialog.select(control.addonName, display_list)
