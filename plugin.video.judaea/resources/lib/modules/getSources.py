...
from resources.lib.modules import all_debrid
from resources.lib.modules.windows.get_sources_window import GetSources as DisplayWindow
...
class Sources(DisplayWindow):
    ...
    def getSources(self, args):
        try:
            ...
            self.args = control.get_item_information(args)
            if self.args is None:
                # Support old format of URLs
                self.args = json.loads(control.unquote(args))
                self.args['info'] = self.args['episodeInfo'].pop('info')

            control.log('Starting Scraping', 'debug')
            ...
            try:
                if control.getSetting('general.torrentCache') == 'true':
                    self.setText(control.lang(32081))
            ...
            if not self.prem_terminate():
                ...
                self.setText(control.lang(32082))
                self.initProviders()
                ...
                control.log('Entering Keep Alive', 'info')
                start_time = time.time()
                runtime = 0

                while self.progress < 100:

                    control.log('Remainin Providers %s' % self.remainingProviders)
                ...
                control.log('Exited Keep Alive', 'info')
            ...
            if len(self.torrentCacheSources) + len(self.hosterSources) + len(self.cloud_files) == 0:
                try:
                    ...
                    pass
                if self.silent:
                    control.showDialog.notification(control.addonName, control.lang(32085))
    ...
    def _torrents_enabled(self):
        if (control.getSetting('premiumize.torrents') == 'true' and control.premiumize_enabled())\
                or (control.getSetting('rd.torrents') == 'true' and control.real_debrid_enabled())\
                or (control.getSetting('alldebrid.torrents') == 'true' and control.all_debrid_enabled()):
            return True
        else:
            return False

    def _hosters_enabled(self):
        if (control.getSetting('premiumize.hosters') == 'true' and control.premiumize_enabled())\
                or (control.getSetting('rd.hosters') == 'true' and control.real_debrid_enabled())\
                or (control.getSetting('alldebrid.hosters') == 'true' and control.all_debrid_enabled()):
            return True
        else:
            return False
    ...
    def build_cache_assist(self):
        ...
        if control.getSetting('general.autocache') == 'false':
            return
        ...
        if control.getSetting('general.cacheAssistMode') == "0":
            ...
            yesno = control.showDialog.yesno('%s - %s' % (control.addonName, control.lang(40307)), control.lang(32086))
            if yesno == 0:
                return

            sorted_list = sorted(self.allTorrents, key=lambda i: i['seeds'], reverse=True)
            display_list = ['%sS | %s | %s | %s' %
                            (i['seeds'], control.color_quality(i['quality']),
                             control.source_size_display(i['size']),
                             control.colorString(i['release_title']))
                            for i in sorted_list]

            selection = control.showDialog.select('%s - ' % control.addonName + control.lang(32087),
                                                display_list)
            if selection == -1:
                return
        ...
        if len(build_list) > 0:
            ...
            actionArgs = control.quote(json.dumps(actionArgs))
            control.execute('RunPlugin(%s?action=cacheAssist&actionArgs=%s)' % (sysaddon, actionArgs))
    ...
    def getTorrent(self, info, provider):
        ...
        try:
            ...
            if len(torrent_results) > 0:
                ...
                control.log('%s cache check took %s seconds' % (provider_name, time.time() - start_time))
    ...
    def getHosters(self, info, provider):
        ...
        try:
            ...
            if sources is None:
                control.log('%s: Found No Sources' % provider_name, 'info')
                return
    ...
    def sortSources(self, torrent_list, hoster_list):
        ...
        if control.getSetting('general.lowQualitysort') == 'true':
            torrent_list = [i for i in torrent_list if 'CAM' not in i['info']] + \
        ...
        if control.getSetting('general.hidesd') == 'true':
    ...
    def updateProgress(self):
        ...
        string1 = '%s - 4K: %s | 1080: %s | 720: %s | SD: %s' % (control.lang(32088),
                                                                 self.colorNumber(list1[0]),
                                                                 self.colorNumber(list1[1]),
                                                                 self.colorNumber(list1[2]),
                                                                 self.colorNumber(list1[3]))

        string2 = '%s - 4k: %s | 1080: %s | 720: %s | SD: %s' % (control.lang(32089),
                                                                 self.colorNumber(list2[0]),
                                                                 self.colorNumber(list2[1]),
                                                                 self.colorNumber(list2[2]),
                                                                 self.colorNumber(list2[3]))

        string4 = '%s - 4k: 0 | 1080: 0 | 720: 0 | SD: 0' % control.lang(32090)
        providerString = ''
        for i in self.remainingProviders:
            providerString += ', ' + control.colorString(str(i))
        string3 = '%s - %s' % (control.lang(32091), providerString[2:])
    ...
    def prem_terminate(self):
        if control.getSetting('preem.cloudfiles') == 'true':
            if len(self.cloud_files) > 0:
                return True

        if 'showInfo' in self.args:
            prem_min = int(control.getSetting('preem.tvres')) + 1
        else:
            prem_min = int(control.getSetting('preem.movieres')) + 1
        if control.getSetting('preem.enabled') == 'false':
            return False
        ...
        limit = int(control.getSetting('preem.limit'))
        type = int(control.getSetting('preem.type'))
        try:
            if type == 0:
                try:
                ...
                if len([i for i in sources if i['quality'] in prem_resolutions]) >= limit:
                    control.log('Pre-emptively Terminated', 'info')
                    return True
            if type == 1:
                try:
...
class TorrentCacheCheck:
    ...
    def torrentCacheCheck(self, torrent_list, info):
        ...
        if control.getSetting('realdebrid.enabled') == 'true' and \
                control.getSetting('rd.torrents') == 'true':
            self.threads.append(
                threading.Thread(target=self.realdebridWorker, args=(copy.deepcopy(torrent_list), info)))

        if control.getSetting('premiumize.enabled') == 'true' and \
                control.getSetting('premiumize.torrents') == 'true':
            self.threads.append(threading.Thread(target=self.premiumizeWorker, args=(copy.deepcopy(torrent_list),)))

        if control.getSetting('alldebrid.enabled') == 'true' and \
                control.getSetting('alldebrid.torrents') == 'true':
