...
class judaeaPlayer(control.player):
    ...
    def start_playback(self):
        ...
        try:
            ...
            if control.playList.size() == control.playList.getposition() + 1 and self.media_type == 'episode':
                smartPlay.SmartPlay(self.original_action_args).append_next_season()

            if self.media_type == 'episode' and control.getSetting('smartplay.upnext') == 'true':
                self.registerUpNext()
    ...
    def registerUpNext(self):
        source_id = 'plugin.video.%s' % control.addonName.lower()
        return_id = 'plugin.video.%s_play_action' % control.addonName.lower()
    ...
    def getWatchedPercent(self, offset=None):
        ...
        control.log('CP: %s, TL: %s' % (current_position, total_length))
        ...
        if int(total_length) is not 0:
            try:
                ...
                traceback.print_exc()
                pass
        control.log('WP: %s' % watched_percent)
    ...
    def keepAlive(self):
        ...
        if self.offset is not None and int(self.offset) != 0 and self.playback_resumed is False:
            control.log("Seeking %s seconds" % self.offset, 'info')
    ...
    def traktBookmark(self):
        if control.playList.getposition() != 0:
            return
        if control.getSetting('smartPlay.traktresume') != 'true':
            return
        if not self.trakt_integration():
            return

        control.log('Getting Trakt Resume Point', 'info')
        ...
        try:
            ...
            if control.getSetting('smartPlay.bookmarkprompt') == 'true':
                if offset is not None and offset is not 0:
                    prompt = control.showDialog.yesno('{}: {}'.format(control.addonName, control.lang(40308)), '%s %s' %
                                                    (control.lang(32092),
                ...
                self.offset = offset
                control.log('Offset is equal to : %s seconds' % offset)
        except:
            import traceback
            traceback.print_exc()
    ...
    def trakt_integration(self):
        ...
        if control.getSetting('trakt.auth') == '':
            ...
            if control.getSetting('trakt.scrobbling') == 'true':
                return True
    ...
    def next_info(self):
        ...
        next_info = control.get_item_information(params.get('actionArgs'))
        ...
        next_info = {
            ...
            "play_info": play_info,
            "notification_time": int(control.getSetting('smartplay.upnexttime'))
        }
