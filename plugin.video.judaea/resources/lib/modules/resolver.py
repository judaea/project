...
class Resolver(BaseWindow):
    ...
    def resolve(self, sources, args, pack_select=False):
        try:
            ...
            control.log('Attempting to Resolve file link', 'info')
            for i in sources:
                ...
                try:
                    ...
                    if i['type'] == 'torrent':
                        ...
                        if 'debrid_provider' in i:
                            if i['debrid_provider'] == 'premiumize' and control.premiumize_enabled():
                                ...
                                if stream_link is None:
                                    ...
                                    try:
                                        ...
                                        control.log('Head Request failed link might be dead, skipping')
                                        continue

                            if i['debrid_provider'] == 'real_debrid' and control.real_debrid_enabled():
                                stream_link = self.realdebridResolve(i, args)
                                if stream_link is None:
                                    continue
                                try:
    ...
    def doModal(self, sources, args, pack_select):
        ...
        thread.start()
        if not self.silent:
            control.dialogWindow.doModal(self)
        else:
            thread.join()
