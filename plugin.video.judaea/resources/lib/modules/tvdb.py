...
from resources.lib.modules import fanarttv, tmdb
from resources.lib.modules import database


class TVDBAPI:
    def __init__(self):
        self.apiKey = control.getSetting('tvdb.apikey')
        if self.apiKey == '':
            self.apiKey = "CAC2F74D9CB97149"

        self.baseUrl = 'https://api.thetvdb.com/'
        self.jwToken = control.getSetting('tvdb.jw')
        ...
        self.tvshows_poster_limit = int(control.getSetting('tvshows.poster_limit'))
        self.tvshows_fanart_limit = int(control.getSetting('tvshows.fanart_limit'))
        self.tvshows_keyart_limit = int(control.getSetting('tvshows.keyart_limit'))
        self.tvshows_characterart_limit = int(control.getSetting('tvshows.characterart_limit'))
        self.tvshows_banner = control.getSetting('tvshows.banner')
        self.season_poster = control.getSetting('season.poster')
        self.season_banner = control.getSetting('season.banner')
        self.season_landscape = control.getSetting('season.landscape')
        self.season_fanart = control.getSetting('season.fanart')
        self.episode_fanart = control.getSetting('episode.fanart')

        self.tvshows_prefer_fanart = control.getSetting('tvshows.preferedsource') == '0'

        if control.fanart_api_key == '':
    ...
    def refresh_lock(self):
        if not control.tvdb_refreshing:
            return False
        for i in range(0, 5):
            if control.tvdb_refresh == '':
                time.sleep(1)
            else:
                self.jwToken = control.tvdb_refresh
                return True
    ...
    def renewToken(self):
        ...
        if 'Error' in response:
            ...
            control.tvdb_refresh = self.jwToken
    ...
    def newToken(self, ignore_lock=False):
        ...
        control.log('Refreshed TVDB Token')
        control.setSetting('tvdb.expiry', str(time.time() + (24 * (60 * 60))))
        return response
    ...
    def seriesIDToListItem(self, trakt_object):
        try:
            ...
            try:
                ...
                pass
            try:
                info['trailer'] = control.youtube_url % trakt_object['trailer'].split('v=')[1]
            except:
                pass
