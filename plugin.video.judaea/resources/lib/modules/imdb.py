...
class IMDBScraper:
    ...
    def showToListItem(self, trakt_object):
        ...
        try:
            ...
            aired = control.datetime_workaround('%s %s %s' % (aired[0], aired[1], aired[2]), '%d %B %Y')
    ...
    def showSeasonToListItem(self, trakt_object, show_meta):
        try:
            ...
            try:
                ...
                aired = aired.split(' ')
                aired[0] = aired[0].zfill(2)
                aired = control.datetime_workaround('%s %s %s' % (aired[0], aired[1].rstrip('.'), aired[2]), '%d %b %Y')
    ...
    def episodeIDToListItem(self, trakt_object, show_meta):
        ...
        try:
            ...
            pass
        try:
            info['title'] = info['originaltitle'] = control.unquote(episode.find('a', {'itemprop': 'name'}).text.strip())
    ...
    def movieToListItem(self, trakt_object):
        try:
            ...
            try:
                ...
                title = control.unquote(title).split('(')[0]
