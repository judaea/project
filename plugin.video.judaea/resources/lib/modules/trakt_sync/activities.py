...
class TraktSyncDatabase(trakt_sync.TraktSyncDatabase):
    ...
    def sync_activities(self):
        ...
        if str(self.activites['all_activities']) == self.base_date and control.getSetting('trakt.auth') != '':
            # Increase the amount of concurrent tasks running during initial and Force Sync processes to speed up task
            control.showDialog.textviewer(control.addonName, control.lang(40133))
            confirmation = control.showDialog.yesno(control.addonName, control.lang(40134))
        ...
        try:
            ...
            if not self.silent:
                self.progress_dialog = control.bgProgressDialog()
                self.progress_dialog.create(control.addonName + 'Sync', 'Judaea: Trakt Sync')
            ...
            if trakt_activities is None:
                control.log('Unable to connect to Trakt', 'error')
                if self.progress_dialog is not None:
                    self.progress_dialog.close()
                    self.progress_dialog = None
                return
