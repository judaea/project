...
class SourceSelect(BaseWindow):
    ...
    def onInit(self):
        ...
        for idx, i in enumerate(self.sources):
            menu_item = control.menuItem(label=('%s) %s' % (idx + 1, i['release_title'])))
            for info in i.keys():
                try:
                    value = i[info]
                    if type(value) == list:
    ...
    def resolve_item(self):
        ...
        self.stream_link = database.get(resolver.doModal, 1, sources,
                                        control.get_item_information(self.actionArgs), False)

        if self.stream_link is None:
            control.showDialog.notification(control.addonName, control.lang(32047), time=2000)
            return
        else:
            self.close()
