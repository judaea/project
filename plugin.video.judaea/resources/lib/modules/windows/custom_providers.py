...
class CustomProviders(BaseWindow):
    ...
    def fill_packages(self):
        ...
        for i in self.packages:
            item = control.menuItem(label=i['pack_name'])
            for info in i.keys():
    ...
    def fill_providers(self, package_name=None):
        ...
        for provider_type in provider_types:
            ...
            for i in [provider for provider in self.providers
                ...
                item = control.menuItem(label=i['provider_name'])
                for info in i.keys():
                    item.setProperty(info, i[info])
        ...
        if action == 7:
            ...
            if focus_id == 3010:
                try: package = self.package_list.getSelectedItem().getLabel()
                    ...
                    return
                control.showBusyDialog()
                confirm = control.showDialog.yesno(control.addonName, control.lang(40255) % package)
                if not confirm:
                    control.closeBusyDialog()
                    return
