...
#     theme = appearance()
#     if theme in ['-', '']: return
#     elif condVisibility('System.HasAddon(script.exegesis.artwork)'):
#         return os.path.join(xbmcaddon.Addon('script.exegesis.artwork').getAddonInfo('path'), 'resources', 'media', theme)
# 
# 
# def appearance():
#     appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.exegesis.artwork)') else setting('appearance.alt').lower()
#     return appearance
# 
# 
# def artwork():
#     execute('RunPlugin(plugin://script.exegesis.artwork)')


def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.exegesis').getAddonInfo('path'), 'resources', 'artwork', theme)
