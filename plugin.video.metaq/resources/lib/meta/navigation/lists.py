...
def lists_trakt_add_all_to_library(user, slug):
    ...
    dialogs.notify(msg='Conversion done', title='starting batch-add', delay=3000, image=get_icon_path("metaq"))
            ...
            if "'tvdb': " in str(item) or "'tvdb_id': " in str(item):
                ...
                if not tmdb or tmdb == None or tmdb =="None":
                    ...
                    if not imdb and not tvdb_id:
                        return dialogs.notify(msg="No id found for item", title='', delay=3000, image=get_icon_path("metaq"))
...
def write_list_id_files(misc_ids, tv_ids, movie_ids, slug, user):
    if len(misc_ids) > 0:
        misc_list_file_path = "special://profile/addon_data/plugin.video.metaq/misc_from_" + slug + "_by_" + user + ".txt"
    ...
    if len(tv_ids) > 0:
        shows_list_file_path = "special://profile/addon_data/plugin.video.metaq/shows_from_" + slug + "_by_" + user + ".txt"
        ...
        if not xbmcvfs.exists(shows_list_file_path):
            ...
            batch_add_file.close()
    if len(movie_ids) > 0:
        movies_list_file_path = "special://profile/addon_data/plugin.video.metaq/movies_from_" + slug + "_by_" + user + ".txt"
...
def write_batch_id_files(tv_ids, movie_ids, misc_ids):
    if len(tv_ids) > 0:
        shows_import_file_path = "special://profile/addon_data/plugin.video.metaq/shows_to_add.txt"
    ...
    if len(movie_ids) > 0:
        movies_import_file_path = "special://profile/addon_data/plugin.video.metaq/movies_to_add.txt"
    ...
    if len(misc_ids) > 0:
        misc_import_file_path = "special://profile/addon_data/plugin.video.metaq/misc_to_add.txt"
...
def lists_trakt_add_liked_to_library():
    ...
    pDialog.create('MetaQ', 'Creating batch import files')
    ...
    pDialog.close()
    write_batch_id_files(tv_ids, movie_ids, misc_ids)
    dialogs.notify(msg='Generating', title='.strm-files', delay=3000, image=get_icon_path("metaq"))
    xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metaq/movies/batch_add_to_library)")
...
def lists_trakt_add_my_lists_to_library():
    ...
    import xbmcgui
    lists = trakt.trakt_get_lists()
    dialogs.notify(msg='Adding ' + str(len(lists)) + " lists", title='to Kodi library', delay=3000, image=get_icon_path("metaq"))
    for list in lists:
        user = list["user"]["username"]
        slug = list["ids"]["slug"]
        items = lists_trakt_show_list(user, slug)
