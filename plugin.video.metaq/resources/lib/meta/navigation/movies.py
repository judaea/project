...
def movies_batch_add_to_library():
    ...
    if xbmcvfs.exists(movie_batch_file):
        ...
        if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)):
            xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metaq/tv/batch_add_to_library)")
            return True
        else:
            xbmc.sleep(1000)
            ...
            scan_library(type="video")
            return True
    if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)): xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metaq/tv/batch_add_to_library)")
...
def make_movie_item(movie_info, is_list = False):
    ...
    if movie_info['poster'] == None or movie_info['poster'] == "": movie_info['poster'] = "https://raw.githubusercontent.com/htpctv/style/master/MetaQ/default/unavailable.png"
