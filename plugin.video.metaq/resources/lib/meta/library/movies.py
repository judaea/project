...
def setup_library(library_folder):
    if library_folder[-1] != "/":
        library_folder += "/"
    metaq_playlist_folder = "special://profile/playlists/mixed/MetaQ/"
    if not xbmcvfs.exists(metaq_playlist_folder): xbmcvfs.mkdir(metaq_playlist_folder)
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set MetaQ as a movies video source?")
        if dialogs.yesno(_("Library setup"), msg):
            source_thumbnail = get_icon_path("movies")
            source_name = "MetaQ " + _("Movies")
