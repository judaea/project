...
def add_tvshow_to_library(library_folder, show, play_plugin = None):
    ...
    if not xbmcvfs.exists(playlist_folder):
        ...
        except: dialogs.notify(msg=_('Creation of MetaQ Playlist Folder'), title=_('Failed'), delay=5000, image=get_icon_path("lists"))
...
def setup_library(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set MetaQ as a tv shows source?")
        if dialogs.yesno("{0} {1}".format(_("Library"), "setup"), msg):
            try:
                source_thumbnail = get_icon_path("tv")
                source_name = "MetaQ " + _("TV shows")
