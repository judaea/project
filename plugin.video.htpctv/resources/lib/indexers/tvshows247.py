...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2p1ZGFlYS9wcm9qZWN0L21hc3Rlci9tZW51cy9saXN0cy8yNDcudHh0')
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.tvshows247_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.tvshows247_list('', result=result)
    ...
    def addSearch(self, url):
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.tvshows247_list, link))
