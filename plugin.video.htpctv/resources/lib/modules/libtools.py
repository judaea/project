...
class libmovies:
    def __init__(self):
        ...
        self.check_setting = control.setting('check_library_movie') or 'false'
    ...
    def add(self, name, title, year, imdb, tmdb, range=False):
        if not control.condVisibility('Window.IsVisible(infodialog)') and not control.condVisibility('Player.HasVideo'):
            control.infoDialog(control.lang(32421).encode('utf-8'), time=10000000)
        ...
        if self.infoDialog == True:
            control.infoDialog(control.lang(32408).encode('utf-8'), time=1)
...
class libepisodes:
    ...
    def update(self, query=None, info='true'):
        ...
        if info == 'true' and not control.condVisibility('Window.IsVisible(infodialog)') and not control.condVisibility('Player.HasVideo'):
            control.infoDialog(control.lang(32422).encode('utf-8'), time=10000000)
            self.infoDialog = True
    ...
    def service(self):
        try:
            lib_tools.create_folder(os.path.join(control.transPath(control.setting('movie_library')), ''))
            lib_tools.create_folder(os.path.join(control.transPath(control.setting('tv_library')), ''))
        except:
            pass
