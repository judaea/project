...
def torrent_status_browser(files):
    ...
    david_cache.DavidCache().set('furk_active_downloads', [i['info_hash'] for i in files], expiration=timedelta(hours=1))
    xbmcplugin.setContent(__handle__, 'files')
    xbmcplugin.endOfDirectory(__handle__)
    setView('view.premium')
...
def furk_tfile_audio():
    ...
    if not item_path_list:
        if dialog.yesno("David Music Player", 'Browse Songs or Play Full Album?', '', '', 'Play Now','Browse'):
            return browse_audio_album(t_files, params.get('name'))
        from modules.player import DavidPlayer
        DavidPlayer().playAudioAlbum(t_files, params.get('name'))
        return browse_audio_album(t_files, params.get('name'))
...
def browse_audio_album(t_files=None, name=None):
    ...
    if seperate:
        if dialog.yesno("David Music Player", 'Browse Songs or Play Full Album?', '', '', 'Play Now','Browse'):
            ...
            from modules.player import DavidPlayer
            DavidPlayer().playAudioAlbum(t_files, from_seperate=True)
    else:
        for item in t_files:
            build_list_object()
        xbmcplugin.setContent(__handle__, 'files')
...
def remove_from_downloads(name='', item_id=''):
    ...
    if resp:
        ...
        if response['status'] == 'ok':
            from datetime import timedelta
            from modules import david_cache
            david_cache.DavidCache().set('furk_active_downloads', None, expiration=timedelta(hours=1))
            dialog.ok('David', '[COLOR=green][B]SUCCESS.[/B][/COLOR]', '[B][I]%s[/I][/B]' % name, 'Torrent removed')
        else:
            dialog.ok('David', '[COLOR=red][B]FAIL.[/B][/COLOR]', '[B][I]%s[/I][/B]' % name, 'Error removing torrent')
        return (None, None)
        dialog.close()
    else: return
...
def add_uncached_file(name=None, item_id=None):
    ...
    name = params.get('name', name)
    item_id = params.get('id', item_id)
    if not dialog.yesno("David", '[B][I]%s[/I][/B]' % name, '', 'Add this uncached torrent to your Furk Files?', 'No','Yes'):
        return
    try:
        ...
        if response['status'] == 'ok':
            ...
            from modules import david_cache
            david_cache.DavidCache().set('furk_active_downloads', None, expiration=timedelta(hours=1))
            dialog.ok('David', '[COLOR=green][B]SUCCESS.[/B][/COLOR]', '[B][I]%s[/I][/B]' % name, 'Torrent added')
        elif response['status'] == 'error':
            dialog.ok('David', '[COLOR=red][B]FAIL.[/B][/COLOR]', '[B][I]%s[/I][/B]' % name, 'Error adding torrent')
            return
    except: return
...
def account_info():
    try:
        ...
        return dialog.ok('David', 'Error Getting Furk Info..', e)
