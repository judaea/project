...
class Navigator:
    ...
    def folder_navigator(self):
        ...
        try:
            ...
            for item in files: make_directory(False)
        except: pass
    ...
    def tips(self):
        ...
        important = ['HELP!!!', 'NEW!!!', 'SPOTLIGHT!!!']
        for tip in files:
            tip_name = tip.replace('.txt', '')[4:]
            if any(i in tip_name for i in important):
                for idx, string in enumerate(important):
    ...
    def because_you_watched(self):
        ...
        def _convert_david_watched_episodes_info():
            final_list = []
            used_names = []
        ...
        func = get_watched_info_movie if db_type == 'movie' else _convert_david_watched_episodes_info
    ...
    def adjust_main_lists(self, params=None):
        ...
        def db_execute():
            ...
            dbcon.commit()
            window.setProperty('david_%s_edited' % list_name, json.dumps(li))
        ...
        def db_execute_shortcut_folder(action='add'):
            ...
            dbcon.commit()
            window.setProperty('david_%s_shortcut_folder' % menu_name, json.dumps(li))
        def db_execute_add_to_shortcut_folder():
            ...
            if action == 'add': dbcon.execute("INSERT INTO navigator VALUES (?, ?, ?)", (menu_name, 'shortcut_folder', json.dumps(li)))
            dbcon.commit()
            window.setProperty('david_%s_shortcut_folder' % list_name, json.dumps(li))
        ...
        try:
            if method == 'display_edit_menu':
                ...
                dbcon.commit()
                window.setProperty('david_%s_%s' % (list_name, list_type), json.dumps(li))
                ...
                if default_list != new_contents:
                    ...
                    if not dialog.yesno("David", "New item [B]%s[/B] Exists." % new_entry.get('name'), "Would you like to add this to the Menu?", '', 'Yes', 'No') == 0: return
                    ...
                    if edited_list:
                        ...
                        window.setProperty('david_%s_edited' % list_name, json.dumps(edited_list))
                    ...
                    window.setProperty('david_%s_default' % list_name, json.dumps(default_list))
                    dbcon.commit()
                    dbcon.close()
                else:
                    return dialog.ok('David', 'No New Items for [B]%s[/B].' % list_name.upper())
            elif method == 'restore':
                ...
                dbcon.commit()
                for item in ('edited', 'default'): window.clearProperty('david_%s_%s' % (list_name, item))
    ...
    def adjust_shortcut_folder_lists(self, params=None):
        ...
        try:
            if method == 'display_edit_menu':
                ...
                if not dialog.yesno("David", "Are you sure?", "Continuing will delete your [B]%s[/B] folder" % list_name) != 0: return
                dbcon = database.connect(NAVIGATOR_DB)
                dbcur = dbcon.cursor()
                dbcur.execute("DELETE FROM navigator WHERE list_name = ?", (list_name,))
                dbcon.commit()
                dialog.ok('DAVID Shortcut Folders', 'You Must Also Remove Instances of this Shortcut Folder from any Lists Individually.')
            elif method == 'remove_all_shortcut_folders':
                if not dialog.yesno("David", "Are you sure?", "Continuing will delete all your Source Folders") == 0: return
                dbcon = database.connect(NAVIGATOR_DB)
                dbcur = dbcon.cursor()
                dbcon.execute("DELETE FROM navigator WHERE list_type=?", ('shortcut_folder',))
                dbcon.commit()
                dialog.ok('DAVID Shortcut Folders', 'You Must Also Remove Instances of Shortcut Folders from any Lists Individually.')
            notification("Process Successful", time=1500)
            xbmc.sleep(200)
    ...
    def _changelog_info(self):
        ...
        changelog_file, changelog_heading = xbmc.translatePath(os.path.join(addon_dir, "resources", "text", "changelog.txt")), '[B]David Changelog[/B]  [I](v.%s)[/I]' % addon_version
        window.setProperty('DAVID_changelog_shown', 'true')
    ...
    def _db_lists(self, list_name=None):
        ...
        try:
            default_contents = json.loads(window.getProperty('david_%s_default' % list_name))
            try: edited_contents = json.loads(window.getProperty('david_%s_edited' % list_name))
        ...
        try:
            ...
            window.setProperty('david_%s_default' % list_name, json.dumps(default_contents))
            window.setProperty('david_%s_edited' % list_name, json.dumps(edited_contents))
            return default_contents, edited_contents
        except:
    ...
    def _db_lists_shortcut_folder(self, list_name=None):
        list_name = self.list_name if not list_name else list_name
        try:
            contents = json.loads(window.getProperty('david_%s_shortcut_folder' % list_name))
            return contents
        except: pass
        try:
            dbcon = database.connect(NAVIGATOR_DB)
            ...
            window.setProperty('david_%s_shortcut_folder' % list_name, json.dumps(contents))
            return contents
        except:
            return []
