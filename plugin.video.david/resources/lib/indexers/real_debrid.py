...
default_rd_icon = os.path.join(icon_directory, 'tools.png')
...
def rd_torrent_cloud():
    ...
    for count, item in enumerate(my_cloud_files, 1):
        try:
            ...
            link_folders_add = {'mode': 'debrid_link_folders', 'debrid_service': 'RD', 'folder_name': normalize(folder_name), 'action': 'add'}
            link_folders_remove = {'mode': 'debrid_link_folders', 'debrid_service': 'RD', 'folder_name': normalize(folder_name), 'action': 'remove'}
...
def browse_rd_cloud(folder_id):
    ...
    except: return xbmcgui.Dialog().ok('David - Real Debrid', 'Cannot display Malformed Pack.')
...
def rd_account_info():
    ...
    try:
        ...
        return dialog.ok('David', 'Error Getting Real Debrid Info..', str(e))
