...
class Discover:
    def __init__(self, db_type=None):
        ...
        if self.db_type: self.window_id = 'DAVID_%s_discover_params' % self.db_type.upper()
        else: self.window_id = ''
    ...
    def similar_recommended(self, key):
        ...
        title = dialog.input("DAVID DISCOVER: Enter Title").lower()
        if not title: return
        ...
        if len(results) == 0: return dialog.ok('DAVID DISCOVER', 'No Matching Titles to Select.', 'Please Try a Different Search Term.')
        choice_list = []
        for item in results:
        ...
        chosen_title = dialog.select("DAVID DISCOVER: Select Correct Title", choice_list, useDetails=True)
    ...
    def include_keywords(self):
        ...
        if keyword:
            ...
            try:
                ...
                keywords_choice = self._multiselect_dialog('DAVID DISCOVER: Choose Included Keywords', [i['name'].upper() for i in result], result)
                if keywords_choice:
    ...
    def exclude_keywords(self):
        ...
        if keyword:
            ...
            try:
                result = tmdb_keyword_id(keyword)['results']
                keywords_choice = self._multiselect_dialog('DAVID DISCOVER: Choose Excluded Keywords', [i['name'].upper() for i in result], result)
                if keywords_choice:
                    for i in keywords_choice:
                        current_key_ids.append(str(i['id']))
                        current_keywords.append(i['name'].upper())
    ...
    def year_start(self):
        ...
        year_start = self._selection_dialog(years_list, years, 'DAVID DISCOVER: Choose Start Year')
        if year_start:
    ...
    def year_end(self):
        ...
        years = years()
        years_list = [str(i) for i in years]
        year_end = self._selection_dialog(years_list, years, 'DAVID DISCOVER: Choose End Year')
        if year_end:
            if self.discover_params['db_type'] == 'movie':
    ...
    def include_genres(self):
        ...
        genres_choice = self._multiselect_dialog('DAVID DISCOVER: Choose Included Genres', [i[0] for i in genre_list], genre_list)
        if genres_choice:
    ...
    def exclude_genres(self):
        ...
        if self.discover_params['db_type'] == 'movie':
            ...
            from modules.nav_utils import tvshow_genres as genres
        genre_list = [(k, v[0]) for k,v in sorted(genres.items())]
        genres_choice = self._multiselect_dialog('DAVID DISCOVER: Choose Excluded Genres', [i[0] for i in genre_list], genre_list)
        if genres_choice:
            genre_ids = ','.join([i[1] for i in genres_choice])
            genre_names = ', '.join([i[0] for i in genres_choice])
    ...
    def language(self):
        ...
        language = self._selection_dialog(languages_list, languages, 'DAVID DISCOVER: Choose Language')
        if language:
    ...
    def region(self):
        ...
        region = self._selection_dialog(region_names, region_codes, 'DAVID DISCOVER: Choose Region')
        if region:
    ...
    def rating(self):
        ...
        rating = self._selection_dialog(ratings_list, ratings, 'DAVID DISCOVER: Choose Minimum Rating')
        if rating:
    ...
    def rating_votes(self):
        ...
        rating_votes = self._selection_dialog(rating_votes_list, rating_votes, 'DAVID DISCOVER: Choose Minimum Number of Votes')
        if rating_votes:
    ...
    def certification(self):
        ...
        certification = self._selection_dialog(certifications_list, certifications, 'DAVID DISCOVER: Choose Certification')
        if certification:
    ...
    def cast(self):
        ...
        from modules.david_cache import cache_object
        result = None
        actor_id = None
        search_name = None
        search_name = dialog.input('DAVID DISCOVER: Enter Actor/Actress Name', type=xbmcgui.INPUT_ALPHANUM)
        ...
        if len(result) > 1:
            ...
            selection = dialog.select("DAVID DISCOVER: Select Correct Actor/Actress", actor_list, useDetails=True)
            if selection >= 0:
    ...
    def network(self):
        ...
        selection = dialog.select("DAVID DISCOVER: Select Network", network_list, useDetails=True)
        if selection >= 0:
    ...
    def companies(self):
        ...
        if company:
            ...
            try:
                ...
                company_choice = self._multiselect_dialog('DAVID DISCOVER: Choose Included Companies', [i['name'].upper() for i in result], result)
                if company_choice:
    ...
    def sort_by(self):
        ...
        sort_by_value = self._selection_dialog([i[0] for i in sort_by_list], [i[1] for i in sort_by_list], 'DAVID DISCOVER: Choose Sort By')
        if sort_by_value:
    ...
    def adult(self):
        key = 'adult'
        include_adult = self._selection_dialog(('True', 'False'), ('true', 'false'), 'DAVID DISCOVER: Choose Adult Titles Inclusion')
        if include_adult:
    ...
    def history(self, db_type=None, display=True):
        ...
        cache_file = os.path.join(__addon_profile__, "david_cache.db")
        db_type = db_type if db_type else self.db_type
        string = 'david_discover_%s_%%' % self.db_type
        settings.check_database(cache_file)
        dbcon = database.connect(cache_file)
        dbcur = dbcon.cursor()
        dbcur.execute("SELECT id, data FROM davidcache WHERE id LIKE ? ORDER BY rowid DESC", (string,))
    ...
    def remove_from_history(self, data_id=None):
        ...
        dbcon = database.connect(cache_file)
        dbcur = dbcon.cursor()
        dbcur.execute("DELETE FROM davidcache WHERE id=?", (data_id,))
        dbcon.commit()
    ...
    def remove_all_history(self):
        from modules.nav_utils import notification
        if not dialog.yesno('Are you sure?','David will Clear Discover\'s %s History.' % self.db_type.upper()): return
    ...
    def help(self):
        heading = 'DAVID DISCOVER: Help Dialog'
    ...
    def _action(self, key):
        ...
        if key in dict_item['search_name']:
            action = self._selection_dialog(['%s Filter' % action.capitalize(),'Clear Filter'], (action, 'clear'), 'DAVID DISCOVER: Filter Action')
    ...
    def _help_text(self):
        ...
        '\n\n*Alternately, you can select "Export Search", give the search a name, and choose which of the main David lists you wish to save it to ' \
        ...
        '\n\n*David will keep a history of the last 7 days of filtered lists you have made. Select "Discover: History" to re-browse these lists.' \
        '\n\n[B]EXAMPLE 1:[/B]' \
        ...
        '\n    - Select "[B]Browse Results[/B]" to immediately see the results or "[B]Export List[/B]" to export the list to David Root Menu or David Movies Menu etc"' \
...
def set_history(db_type, name, query):
    ...
    string = 'david_discover_%s_%s' % (db_type, query)
