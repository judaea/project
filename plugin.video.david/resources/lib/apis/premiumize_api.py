...
class PremiumizeAPI:
    def __init__(self):
        ...
        self.user_agent = 'David for Kodi'
    ...
    def user_cloud(self, folder_id=None):
        if folder_id:
            string = "david_pm_user_cloud_%s" % folder_id
            url = "folder/list?id=%s" % folder_id
        else:
            string = "david_pm_user_cloud_root"
            url = "folder/list"
        return cache_object(self._get, string, url, False, 2)

    def user_cloud_all(self):
        string = "david_pm_user_cloud_all_files"
        url = "item/listall"
        return cache_object(self._get, string, url, False, 2)

    def transfers_list(self):
        string = "david_pm_transfers_list"
        url = "transfer/list"
        return cache_object(self._get, string, url, False, 2)
    ...
    def get_item_details(self, item_id):
        string = "david_pm_item_details_%s" % item_id
        url = "item/details"
        data = {'id': item_id}
        args = [url, data]
    ...
    def clear_cache(self):
        try:
            import xbmc, xbmcvfs
            import os
            PM_DATABASE = os.path.join(xbmc.translatePath(__addon__.getAddonInfo('profile')), 'david_cache.db')
            ...
            dbcur = dbcon.cursor()
            dbcur.execute("""SELECT id FROM davidcache WHERE id LIKE ?""", ("david_pm_user_cloud%",))
            user_cloud_cache = dbcur.fetchall()
            user_cloud_cache = [i[0] for i in user_cloud_cache]
            for i in user_cloud_cache:
                dbcur.execute("""DELETE FROM davidcache WHERE id=?""", (i,))
