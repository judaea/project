...
class RealDebridAPI:
    ...
    def user_cloud(self):
        string = "david_rd_user_cloud"
        url = "torrents"
        return cache_object(self._get, string, url, False, 2)

    def downloads(self):
        string = "david_rd_downloads"
        url = "downloads"
        return cache_object(self._get, string, url, False, 2)

    def user_cloud_info(self, file_id):
        string = "david_rd_user_cloud_info_%s" % file_id
    ...
    def delete_download(self, download_id):
        ...
        response = requests.delete(self.base_url + url).json()
        return response

    def get_hosts(self):
        hosts_dict = {'Real-Debrid': []}
        hosts = []
        string = "david_rd_valid_hosts"
        url = "hosts/status"
        try:
    ...
    def add_uncached_torrent(self, magnet_url):
        ...
        def _return_failed(message='Unknown Error.'):
            ...
            hide_busy_dialog()
            xbmc.sleep(500)
            xbmcgui.Dialog().ok('DAVID Cloud Transfer', message)
            return False
        show_busy_dialog()
        extensions = supported_video_extensions()
        torrent = self.add_magnet(magnet_url)
        ...
        if status == 'magnet_conversion':
            ...
            timeout = 100
            progressDialog.create('DAVID Cloud Transfer', line1, line2, line3)
    ...
    def _post(self, url, post_data):
        ...
        if '?' not in url:
            ...
            url += "&auth_token=%s" % self.token
        response = requests.post(url, data=post_data, timeout=10).text
        if 'bad_token' in response or 'Bad Request' in response:
            self.refreshToken()
    ...
    def clear_cache(self):
        try:
            import xbmc, xbmcvfs
            import os
            RD_DATABASE = os.path.join(xbmc.translatePath(__addon__.getAddonInfo('profile')), 'david_cache.db')
            ...
            dbcur = dbcon.cursor()
            # USER CLOUD
            dbcur.execute("""SELECT data FROM davidcache WHERE id=?""", ("david_rd_user_cloud",))
            try: 
            ...
            if not user_cloud_success:
                dbcur.execute("""DELETE FROM davidcache WHERE id=?""", ("david_rd_user_cloud",))
                window.clearProperty("david_rd_user_cloud")
                for i in user_cloud_info_caches:
                    dbcur.execute("""DELETE FROM davidcache WHERE id=?""", ("david_rd_user_cloud_info_%s" % i,))
                    window.clearProperty("david_rd_user_cloud_info_%s" % i)
                dbcon.commit()
                user_cloud_success = True
            # DOWNLOAD LINKS
            dbcur.execute("""DELETE FROM davidcache WHERE id=?""", ("david_rd_downloads",))
            window.clearProperty("david_rd_downloads")
            dbcon.commit()
