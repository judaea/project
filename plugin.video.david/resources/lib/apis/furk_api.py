...
class FurkAPI:
    ...
    def search(self, query):
        try:
            ...
            cache_name = "david_FURK_SEARCH_%s" % link
    ...
    def direct_search(self, query):
        try:
            ...
            cache_name = "david_FURK_SEARCH_DIRECT_%s" % link
            cache = _cache.get(cache_name)
            if cache:
                files = cache
            else:
    ...
    def t_files(self, file_id):
        try:
            cache = _cache.get("david_%s_%s" % ('FURK_T_FILE', file_id))
            if cache:
                t_files = cache
            else:
                api_key = self.get_api()
                ...
                _cache.set("david_%s_%s" % ('FURK_T_FILE', file_id), t_files,
                    expiration=datetime.timedelta(hours=2))
            return t_files
        except: return
...
def clear_media_results_database():
    ...
    dbcur = dbcon.cursor()
    dbcur.execute("SELECT id FROM davidcache WHERE id LIKE 'david_FURK_SEARCH_%'")
    try:
        furk_results = [str(i[0]) for i in dbcur.fetchall()]
        if not furk_results: return 'success'
        dbcur.execute("DELETE FROM davidcache WHERE id LIKE 'david_FURK_SEARCH_%'")
        dbcon.commit()
