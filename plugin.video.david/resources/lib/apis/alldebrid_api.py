...
class AllDebridAPI:
    def __init__(self):
        self.user_agent = 'david_for_kodi'
    ...
    def clear_cache(self):
        try:
            import xbmc, xbmcvfs
            import os
            AD_DATABASE = os.path.join(xbmc.translatePath(__addon__.getAddonInfo('profile')), 'david_cache.db')
            ...
            dbcur = dbcon.cursor()
            dbcur.execute("""DELETE FROM davidcache WHERE id=?""", ('david_ad_user_cloud',))
            window.clearProperty('david_ad_user_cloud')
            dbcon.commit()
            dbcon.close()
            return True
        except: return False
