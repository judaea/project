...
def trakt_collection_widgets(db_type, param1, param2):
    ...
    window_property_name = 'david_trakt_collection_%s' % string_insert
...
def trakt_watchlist(db_type, page_no, letter):
    ...
    limit = settings.page_limit()
    string_insert = 'movie' if db_type in ('movie', 'movies') else 'tvshow'
    window_property_name = 'david_trakt_watchlist_%s' % string_insert
    try: original_list = json.loads(window.getProperty(window_property_name))
...
def trakt_fetch_collection_watchlist(list_type, db_type):
    ...
    window_property_name = 'david_trakt_%s_%s' % (list_type, string_insert)
...
def get_trakt_my_calendar():
    ...
    from davidmeta import tvshow_meta, retrieve_user_info
...
def get_trakt_movie_id(item):
    ...
    from davidmeta.tmdb import tmdbMoviesExternalID
    tmdb_id = None
    if item['imdb']:
        try:
...
def get_trakt_tvshow_id(item):
    if item['tmdb']: return item['tmdb']
    from davidmeta.tmdb import tmdbTVShowsExternalID
    tmdb_id = None
    if item['imdb']:
        try: 
...
def sync_watched_trakt_to_david(refresh=False):
    if refresh: window.setProperty('david_trakt_sync_complete', 'false')
    if window.getProperty('david_trakt_sync_complete') == 'true': return
    ...
    try:
        if not_home_window:
            ...
            bg_dialog.create('Trakt & David Watched Status', 'Please Wait')
        ...
        david_watched_movies = dbcur.fetchall()
        david_watched_movies = [int(i[0]) for i in david_watched_movies]
        compare_trakt_movies = [i[0] for i in trakt_watched_movies]
        process_trakt_movies = trakt_watched_movies
        if not sorted(david_watched_movies) == sorted(compare_trakt_movies): process_movies = True
        if not_home_window: bg_dialog.update(50, 'Trakt & David Watched Status', 'Checking Movies Watched Status')
        xbmc.sleep(100)
        dbcur.execute("SELECT media_id, season, episode FROM watched_status WHERE db_type = ?", ('episode',))
        david_watched_episodes = dbcur.fetchall()
        david_watched_episodes = [(int(i[0]), i[1], i[2]) for i in david_watched_episodes]
        for i in trakt_watched_tv:
            for x in i[2]:
                compare_trakt_tv.append((i[0], x[0], x[1]))
                processed_trakt_tv.append((i[0], x[0], x[1], i[3]))
        if not sorted(david_watched_episodes) == sorted(compare_trakt_tv): process_tvshows = True
        if not_home_window: bg_dialog.update(100, 'Trakt & David Watched Status', 'Checking Episodes Watched Status')
        xbmc.sleep(100)
        ...
        if process_movies:
            ...
            for count, i in enumerate(process_trakt_movies):
                try:
                    if not_home_window: bg_dialog.update(int(float(count) / float(len(trakt_watched_movies)) * 100), 'Trakt & David Watched Status', 'Syncing Movie Watched Status')
        ...
        if process_tvshows:
            ...
            for count, i in enumerate(processed_trakt_tv):
                try:
                    if not_home_window: bg_dialog.update(int(float(count) / float(len(processed_trakt_tv)) * 100), 'Trakt & David Watched Status', 'Syncing Episode Watched Status')
                    last_played = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        if not_home_window:
            bg_dialog.close()
            from modules.nav_utils import notification
            notification('Trakt Watched to David Watched Sync Complete', time=4000)
        window.setProperty('david_trakt_sync_complete', 'true')
        __addon__.setSetting('trakt_indicators_active', 'true')
