...
__david__ = xbmcaddon.Addon(id='plugin.video.david')
__external__ = xbmcaddon.Addon(id='script.module.openscrapers')
david_icon = __david__.getAddonInfo('icon')
...
class ExternalSource:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, language):
        ...
        def _background():
            while time.time() < end_time:
                ...
                info = [sourcelabelDict[x.getName()] for x in threads if x.is_alive() is True]
                if len(self.sources) >= 100 * len(info): break
        self.dialog_background = True if (__david__.getSetting('auto_play'), __david__.getSetting('autoplay_minimal_notifications')) == ('true', 'true') else False
        ...
        pre_emp =  __david__.getSetting('preemptive.termination')
        pre_emp_quality = __david__.getSetting('preemptive.quality')
        pre_emp_limit = __david__.getSetting('preemptive.limit')
        try: timeout = int(__david__.getSetting('scrapers.timeout.1'))
        except: timeout = 30
        start_time = time.time()
        end_time = start_time + timeout
        if self.background: _background()
        else:
            int_dialog_highlight = __david__.getSetting('int_dialog_highlight')
            if not int_dialog_highlight or int_dialog_highlight == '': int_dialog_highlight = 'darkgoldenrod'
            ext_dialog_highlight = __david__.getSetting('ext_dialog_highlight')
            if not ext_dialog_highlight or ext_dialog_highlight == '': ext_dialog_highlight = 'dodgerblue'
            finish_early = __david__.getSetting('search.finish.early')
    ...
    def sourcesFilter(self):
        ...
        self.torrentCheckCache = __david__.getSetting('torrent.check.cache')
        if self.torrentCheckCache == '':  self.torrentCheckCache = 'false'
        self.uncachedTorrents = __david__.getSetting('torrent.display.uncached')
        if self.uncachedTorrents == '': self.uncachedTorrents = 'false'
        removeDuplicates = __david__.getSetting('remove.duplicates')
        if removeDuplicates == '': removeDuplicates = 'false'
        include_nondebrid = __david__.getSetting('include_nondebrid_results')
        if include_nondebrid == '':  include_nondebrid = 'true'
        captcha = __david__.getSetting('hosts.captcha')
        if captcha == '': captcha = 'true'
        if removeDuplicates == 'true':
            if len(self.sources) > 0:
                self.sources = list(self.sourcesRemoveDuplicates(self.sources))
                if not self.suppress_notifications: dialog.notification('David', '[B]%d[/B] Duplicate URL%s Removed' % (self.duplicates, ('' if self.duplicates == 1 else 's')) , david_icon, 2500, sound=False)
    ...
    def sourcesUpdate(self, source, sources):
        def _addInfoandName(i):
            extraInfo = external_source_utils.getFileType(i['url'])
            URLName = external_source_utils.getFileNameMatch(self.info['title'], i['url'])
        ...
        for i in sources:
            update_dict = {'provider': source, 'external': True, 'scrape_provider': self.scrape_provider, 'url': i['url']}
    ...
    def sourcesProcessTorrents(self, torrentSources):
        ...
        try:
            ...
            if self.uncachedTorrents == 'true':
                uncachedRDTorrents = [dict(i.items() + [('cache_provider', 'Uncached Real-Debrid')]) for i in torrentSources if not i['info_hash'] in cachedRDHashes]
                uncachedPMTorrents = [dict(i.items() + [('cache_provider', 'Uncached Premiumize.me')]) for i in torrentSources if not i['info_hash'] in cachedPMHashes]
                uncachedADTorrents = [dict(i.items() + [('cache_provider', 'Uncached AllDebrid')]) for i in torrentSources if not i['info_hash'] in cachedADHashes]
    ...
    def sourcesRemoveFailing(self):
        ...
        try:
            ...
            try: threshold = thresholds[int(__david__.getSetting('failing_scrapers.threshold'))]
            except: threshold = 25
            ...
            if len(remove_sources) > 0:
                ...
                if not self.suppress_notifications:
                    dialog.notification('David', '[B]%d[/B] Failing Scraper%s Disabled' % (len(remove_sources), ('' if len(remove_sources) == 1 else 's')) , david_icon, 2500, sound=False)
        except: pass
    ...
    def getConstants(self):
        ...
        sourceDict = sources()
        self.sourceDict = [i for i in sourceDict if not i[0] in ('furk', 'easynews')]
        ...
        self.providerDatabase = os.path.join(xbmc.translatePath(__david__.getAddonInfo('profile')), "ext_providers3.db")
        ...
        self.hostcapDict = ['hugefiles.net', 'hugefiles.cc', 'vidup.me', 'vidup.tv', 'flashx.tv', 'flashx.to', 'flashx.sx', 'flashx.bz',
                            'flashx.cc', 'vshare.eu', 'vshare.io', 'thevideo.me', 'vev.io', 'uptobox.com', 'uptostream.com']
        ...
        self.furk_enabled = __david__.getSetting('provider.furk')
        self.easynews_enabled = __david__.getSetting('provider.easynews')
        self.pm_cloud_enabled = __david__.getSetting('provider.pm-cloud')
        self.rd_cloud_enabled = __david__.getSetting('provider.rd-cloud')
        self.ad_cloud_enabled = __david__.getSetting('provider.ad-cloud')
        self.local_enabled = __david__.getSetting('provider.local')
        self.downloads_enabled = __david__.getSetting('provider.downloads')
        self.folder1_enabled = __david__.getSetting('provider.folder1')
        self.folder2_enabled = __david__.getSetting('provider.folder2')
        self.folder3_enabled = __david__.getSetting('provider.folder3')
        self.folder4_enabled = __david__.getSetting('provider.folder4')
        self.folder5_enabled = __david__.getSetting('provider.folder5')
        self.progressHeading = int(__david__.getSetting('progress.heading'))
        self.remove_failing_sources = __david__.getSetting('remove.failing_scrapers')
        self.internal_activated = True if 'true' in (self.furk_enabled, self.easynews_enabled, self.pm_cloud_enabled, self.rd_cloud_enabled, self.ad_cloud_enabled,
                                                    self.folder1_enabled, self.folder2_enabled, self.folder3_enabled, self.folder4_enabled, self.folder5_enabled) \
                                else False
    ...
    def exportSettings(self):
        try:
            ...
            __external__.setSetting('torrent.min.seeders', __david__.getSetting('torrent.min.seeders'))
        except: pass
