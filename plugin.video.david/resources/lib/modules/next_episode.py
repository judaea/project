...
from davidmeta import tvshow_meta, all_episodes_meta, retrieve_user_info, check_meta_database
    ...
    def _process_eps(item):
        try:
            ...
            episode_item = {"season": season, "episode": episode, "meta": meta, "curr_last_played_parsed": datetime_object, "action": "next_episode",
                            ...
                            "adjust_hours": adjust_hours, "current_adjusted_date": current_adjusted_date}
...
def add_next_episode_unwatched(action=None, media_id=None, silent=False):
    ...
    if action == 'add': command, line1 = "INSERT OR IGNORE INTO unwatched_next_episode VALUES (?)", '%s Added to David Next Episode' % params.get('title')
    else: command, line1 = "DELETE FROM unwatched_next_episode WHERE media_id=?", '%s Removed from David Next Episode' % params.get('title')
    dbcon = database.connect(WATCHED_DB)
...
def add_to_remove_from_next_episode_excludes():
    ...
    if action == 'add':
        ...
        line1 = '[B]{}[/B] excluded from David Next Episode'.format(title.upper())
    elif action == 'remove':
        dbcon.execute("DELETE FROM exclude_from_next_episode WHERE media_id=?", (media_id,))
        line1 = '[B]{}[/B] included in David Next Episode'.format(title.upper())
    dbcon.commit()
    dbcon.close()
...
def build_next_episode_manager():
    ...
    if action == 'manage_unwatched':
        ...
        heading = 'Select Show to remove from David Next Episode:'
...
def next_episode_options_choice(setting=None):
    ...
    choices = [
            ...
            ('Include Trakt or David Unwatched', 'nextep.include_unwatched', [('OFF', 'false'), ('ON', 'true')]),
