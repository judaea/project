    ...
    if db_type == 'tvshow':
        if watched_indicators in (1, 2):
            ...
            for item in watched_list:
                meta = davidmeta.tvshow_meta('tmdb_id', item[0], meta_user_info)
...
def mark_season_as_watched_unwatched():
    ...
    if watched_indicators in (0, 1):
        ...
        meta = davidmeta.tvshow_meta('tmdb_id', media_id, meta_user_info)
        ep_data = davidmeta.season_episodes_meta(media_id, tvdb_id, season, meta['tvdb_summary']['airedSeasons'], meta['season_data'], meta_user_info)
        count = 1
        se_list = []
        for item in ep_data:
...
def mark_tv_show_as_watched_unwatched():
    ...
    if watched_indicators in (0, 1):
        ...
        count = 1
        meta = davidmeta.tvshow_meta('tmdb_id', media_id, meta_user_info)
        season_data  = davidmeta.all_episodes_meta(media_id, tvdb_id, meta['tvdb_summary']['airedSeasons'], meta['season_data'], meta_user_info)
        total = sum([i['episode_count'] for i in season_data if i['season_number'] > 0])
        for item in season_data:
            season_number = item['season_number']
            if season_number <= 0: continue
            ep_data = davidmeta.season_episodes_meta(media_id, tvdb_id, season_number, meta['tvdb_summary']['airedSeasons'], meta['season_data'], meta_user_info)
            for ep in ep_data:
...
def mark_as_watched_unwatched(db_type='', media_id='', action='', season='', episode='', title=''):
    try:
        ...
        from modules.nav_utils import notification
        notification('Error Marking Watched in David', time=5000)
