...
def get_theme():
    if __addon__.getSetting('theme_installed') == 'true':
        theme = __addon__.getSetting('david.theme').lower()
        result = os.path.join(xbmcaddon.Addon('script.david.artwork').getAddonInfo('path'), 'resources', 'media', theme)
    elif addon_installed('script.david.artwork'):
...
def tmdb_api_check():
    ...
    if not tmdb_api or tmdb_api == '':
        tmdb_api = '6e6a68c545dffdce0e054d1060fdd6e0'
    return tmdb_api
...
def auto_start_david():
    if __addon__.getSetting('auto_start_david') == 'true': return True
    else: return False
...
def watched_indicators():
    ...
    if watched_indicators == '1' and __addon__.getSetting('sync_david_watchstatus') == 'true': return 1
    return 2
...
def active_scrapers(group_folders=False):
    ...
    settings = ['provider.external', 'provider.furk', 'provider.easynews', 'provider.pm-cloud', 'provider.rd-cloud', 'provider.ad-cloud',
...
def initialize_databases():
    ...
    TRAKT_DB = os.path.join(DATA_PATH, "david_trakt.db")
    DAVID_DB = os.path.join(DATA_PATH, "david_cache.db")
    ...
    if not xbmcvfs.exists(TRAKT_DB):
        ...
        dbcon.execute("""CREATE TABLE IF NOT EXISTS david_trakt
                           (id text UNIQUE, expires integer, data text)
                            """)
        dbcon.close()
    if not xbmcvfs.exists(DAVID_DB):
        dbcon = database.connect(DAVID_DB)
        dbcon.execute("""CREATE TABLE IF NOT EXISTS davidcache
