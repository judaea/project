...
''' Base code from script.module.simplecache by marcelveldt '''
...
ADDON_ID = "plugin.video.david"


class DavidCache(object):
    ...
    def check_cleanup(self):
        ...
        lastexecuted = self._win.getProperty("david.clean.lastexecuted")
        if not lastexecuted:
            self._win.setProperty("david.clean.lastexecuted", repr(cur_time))
    ...
    def _get_mem_cache(self, endpoint, checksum, cur_time):
        ...
        '''
        result = None
        try: cachedata = self._win.getProperty(endpoint.encode("utf-8"))
        except: cachedata = self._win.getProperty(endpoint)
        if cachedata:
    ...
    def _get_db_cache(self, endpoint, checksum, cur_time):
        ...
        result = None
        query = "SELECT expires, data, checksum FROM davidcache WHERE id = ?"
    ...
    def _set_db_cache(self, endpoint, checksum, expires, data):
        ...
        query = "INSERT OR REPLACE INTO davidcache( id, expires, data, checksum) VALUES (?, ?, ?, ?)"
        data = repr(data)
    ...
    def _do_cleanup(self):
        ...
        if self._win.getProperty("davidcachecleanbusy"):
            return
        self._win.setProperty("davidcachecleanbusy", "busy")

        query = "SELECT id, expires FROM davidcache"
        for cache_data in self._execute_sql(query).fetchall():
            if self._exit or self._monitor.abortRequested():
                return
            # always cleanup all memory objects on each interval
            try: self._win.clearProperty(cache_data[0].encode("utf-8"))
            except: self._win.clearProperty(cache_data[0])
            # clean up db cache object only if expired
            if cache_data[1] < cur_timestamp:
                query = 'DELETE FROM davidcache WHERE id = ?'
        ...
        self._win.setProperty("davidcache.clean.lastexecuted", repr(cur_time))
        self._win.clearProperty("davidcachecleanbusy")
    ...
    def _get_database(self):
        ...
        try: dbfile = xbmc.translatePath("%s/david_cache.db" % dbpath).decode('utf-8')
        except: dbfile = xbmc.translatePath("%s/david_cache.db" % dbpath)
        if not xbmcvfs.exists(dbpath):
            xbmcvfs.mkdirs(dbpath)
        del addon
        try:
            connection = sqlite3.connect(dbfile, timeout=30, isolation_level=None)
            connection.execute('SELECT * FROM davidcache LIMIT 1')
            return connection
            ...
            try:
                connection = sqlite3.connect(dbfile, timeout=30, isolation_level=None)
                connection.execute(
                    """CREATE TABLE IF NOT EXISTS davidcache(
    ...
    def _log_msg(msg, loglevel=xbmc.LOGDEBUG):
        ...
        xbmc.log("Davidcache --> %s" % msg, level=loglevel)
...
def use_cache(cache_days=14):
    '''
        wrapper around our david cache to use as decorator
        Usage: define an instance of Davidcache with name "cache" (self.cache) in your class
