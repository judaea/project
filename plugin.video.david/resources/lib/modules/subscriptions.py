...
class Subscriptions:
    def __init__(self, db_type=None, tmdb_id=None, action=None, orig_mode=None):
        davidmeta.check_meta_database()
        self.db_type = db_type
        self.tmdb_id = tmdb_id #str
        self.action = action
        self.orig_mode = orig_mode
        self.meta_user_info = davidmeta.retrieve_user_info()
        if self.db_type:
    ...
    def update_subscriptions(self, suppress_extras=False):
        ...
        for count, item in enumerate(subscriptions, 1):
            try:
                ...
                display = 'David Updating - [B][I]%s[/I][/B]' % self.rootname
        ...
        xbmc.sleep(500)
        bg_dialog.close()
        notification('David Subscriptions Updated', 4500)
    ...
    def add_trakt_subscription_listitem(self, db_type, ids, count, total, path, dialog):
        try:
            ...
            meta_action = davidmeta.movie_meta if db_type in ('movie', 'movies') else davidmeta.tvshow_meta
            tmdb_id = get_ids(ids)
    ...
    def create_tvshow_strm_files(self, meta=None, folder=None, tmdb_id=None, title=None, year=None):
        ...
        season_data = davidmeta.all_episodes_meta(meta['tmdb_id'], meta['tvdb_id'], meta['tvdb_summary']['airedSeasons'], meta['season_data'], self.meta_user_info)
    ...
    def add_remove_constants(self):
        meta_action = davidmeta.movie_meta if self.db_type == 'movie' else davidmeta.tvshow_meta
...
def subscriptions_add_list(db_type):
    ...
    if xbmcgui.Dialog().yesno('David Subscriptions','Are you sure you have set your David [B]%s[/B] Subscription Folder as a Source for the Kodi Library?' % db_type.upper(), '', '', 'Already Done', 'I\'ll Do It Now'):
    ...
    if not xbmcgui.Dialog().yesno('Are you sure?','David will add all [B]%s[/B] from [B]%s[/B] to your David Subscriptions, and monitor this list for changes. Do you wish to continue?' % (dialog_display, trakt_list['name'])):
    ...
    if len(current_subscriptions) > 0:
        if not xbmcgui.Dialog().yesno('Current Subscriptions Found!','You have items in your [B]%s[/B] Subscription. David will delete these before continuing (may take some time). Do you wish to continue?' % (dialog_display)):
            return open_settings(open_setting)
        clear_library = True
        show_busy_dialog()
    ...
    xbmcgui.Dialog().ok('David Subscriptions', end_dialog % trakt_list['name'])
    if clear_library:
        if xbmcgui.Dialog().yesno('David Subscriptions','Do you wish to clear Kodi\'s Library of your previous Subscription items?'):
            import time
            xbmc.executebuiltin('CleanLibrary(video)')
            time.sleep(3)
            while xbmc.getCondVisibility("Window.IsVisible(ProgressDialog)"):
                time.sleep(1)
    if xbmcgui.Dialog().yesno('David Subscriptions','Do you wish to scan your new Subscription list into Kodi\'s Library?'):
