...
def set_quality(quality_setting=None):
    ...
    if filters == []:
        import xbmcgui
        xbmcgui.Dialog().ok('DAVID Filters', 'You must select at least 1 Filter Setting.', '', 'Please Retry.....')
...
def enable_scrapers():
    ...
    from modules.nav_utils import toggle_setting
    scrapers = ['external', 'furk', 'easynews', 'pm-cloud', 'rd-cloud', 'ad-cloud', 'local', 'downloads', 'folders']
    preselect = [scrapers.index(i) for i in active_scrapers(group_folders=True)]
    scrapers_dialog = [i.upper() for i in scrapers]
    scraper_choice = multiselect_dialog('Choose David Scrapers', scrapers_dialog, scrapers, preselect=preselect)
...
def set_subtitle_action():
    ...
    choice = selection_dialog(choices, choices, 'David - Choose Subtitles Action')
...
def set_display_mode():
    ...
    settings = ('0', '1', '2')
    choice = selection_dialog(names, settings, 'David - Choose Display Mode')
...
def subscriptions_update_interval():
    ...
    if params['update_type'] == 'interval':
        ...
        choice = selection_dialog([i[0] for i in intervals], intervals, 'David - Choose Subscription Update Interval')
        ...
        choice = selection_dialog([i[0] for i in intervals], intervals, 'David - Choose Daily Subscription Update Time')
...
def switch_settings():
    ...
    choice = selection_dialog(choices, choices, 'David - Choose Settings Layout')
    if choice:
...
def folder_sources_choice():
    ...
    params = dict(parse_qsl(sys.argv[2].replace('?','')))
    setting = params['setting']
    folder = xbmcgui.Dialog().browse(0, 'DAVID - Select Directory...', '')
...
def internal_scrapers_order_choice():
    ...
    try: current_ordering = window.getProperty('DAVID_internal_scrapers_order').split(', ')
    ...
    adjust_scraper = selection_dialog([i[0] for i in choices], [i[0] for i in choices], 'David - Choose Scraper to Change Order')
    if not adjust_scraper: return window.clearProperty('DAVID_internal_scrapers_order')
    ...
    positioning_info = selection_dialog([i[0] for i in choices], [i[1] for i in choices], 'David - Choose %s Scraper Position' % adjust_scraper)
    ...
    window.setProperty('DAVID_internal_scrapers_order', new_order_setting)
...
def build_add_to_remove_from_list(meta='', media_type='', orig_mode='', from_search=''):
    ...
    listing = [(heading + 'Trakt List', 'trakt'), (heading + 'David Favourites', 'favourites')]
    if not trakt_list_subscriptions(): listing.append((heading + 'David Subscriptions', 'subscriptions'))
    if media_type == 'tvshow' and watched_indicators() == 0: listing.append((heading + 'David Next Episode', 'unwatched_next_episode'))
...
def playback_menu(from_results=False, suggestion=None, list_name=None, play_params=None):
    ...
    indicators_status, indicators_toggle = ('Trakt', '0') if settings.watched_indicators() in (1, 2) else ('David', '1')
    ...
    listing += [('OPEN DAVID META SETTINGS', 'open_meta_settings')]
    listing += [('OPEN OPENSCRAPERS SETTINGS', 'open_scraper_settings')]
    listing += [('OPEN DAVID SETTINGS', 'open_david_settings')]
    listing += [('[B]SAVE SETTINGS AND EXIT[/B]', 'save_and_exit')]
    xbmc.sleep(500)
    choice = selection_dialog([i[0] for i in listing], [i[1] for i in listing], 'David Options...')
    ...
    if choice in (None, 'rescrape_select', 'save_and_exit', 'clear_cache_page', 'toggle_indicators', 'clear_trakt_cache', 'search_directly', 'open_meta_settings', 'open_scraper_settings', 'open_david_settings', 'open_external_scrapers_manager'): return
