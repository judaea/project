...
class CacheAssit:
    def __init__(self, url):
        url = json.loads(control.unquote(url))
        args = url['args']
        ...
        cache_location = int(control.getSetting('general.cachelocation'))

        threads = []
        self.notified = False

        if cache_location == 0 and control.getSetting('premiumize.enabled') == 'true':
            threads.append(threading.Thread(target=self.premiumize_downloader, args=(torrent_list[0],)))

        elif cache_location == 1 and control.getSetting('realdebrid.enabled') == 'true':
    ...
    def premiumize_downloader(self, torrent_object):
        ...
        try:
            ...
            control.showDialog.notification(control.addonName, control.lang(32072))
            database.add_assist_torrent(transfer_id, 'premiumize', 'queued',
                                        torrent_object['release_title'], str(current_percent))
        except:
            import traceback
            traceback.print_exc()
            control.log('Failed to start premiumize debrid transfer', 'error')
            return
            ...
            try:
                ...
                if transfer_status['status'] == 'finished':
                    ...
                    if not self.notified:
                        control.showDialog.notification(control.addonName + ': %s' % self.title,
                                                      control.lang(40299) % self.title, time=5000)
                ...
                if current_percent == transfer_status['progress']:
                    if timestamp == (time.time() + 10800):
                        database.add_assist_torrent(transfer_id, 'premiumize', 'failed',
                                                    ...
                                                    str(current_percent))
                        debrid.delete_transfer(transfer_id)
                        control.showDialog.notification(control.addonName, control.lang(40300) % self.title)
                        break
                    continue
                else:
                    database.add_assist_torrent(transfer_id, 'premiumize', transfer_status['status'],
        ...
        while not monitor.abortRequested():
            ...
            try:
                ...
                if info['status'] == 'downloaded':
                    control.showDialog.notification(control.addonName + ': %s' % self.title,
                                                  control.lang(32072) + ' %s' % self.title,
                                                  time=5000)
                ...
                if info['status'] in downloading_status:
                    ...
                    database.add_assist_torrent(torrent_id, 'real_debrid', 'failed', torrent_object['release_title'],
                                                str(current_percent))
                    debrid.deleteTorrent(torrent_id)
                    control.log('Could not create cache for magnet- %s' % torrent_object['magnet'], 'info')
                    control.showDialog.notification(control.addonName, control.lang(40301) % self.title)
                    break
            except:
                debrid.deleteTorrent(torrent_id)