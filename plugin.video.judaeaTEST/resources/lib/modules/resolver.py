...
from resources.lib.modules import premiumize as Premiumize
from resources.lib.modules import real_debrid
from resources.lib.modules import all_debrid
from resources.lib.modules.windows.base_window import BaseWindow
...
class Resolver(BaseWindow):
        ...
        try:
            ...
            for i in sources:
                ...
                try:
                    ...
                    if 'size' in i:
                        i['info'].append(control.source_size_display(i['size']))

                    self.setProperty('release_title', control.display_string(i['release_title']))
                    ...
                    if i['type'] == 'torrent':
                        ...
                        if stream_link is None:
                            control.log('Failed to resolve for torrent %s' % i['release_title'])
                            continue
                        else:
                        ...
                        if 'debrid_provider' in i:
                            ...
                            if stream_link is None:
                                ...
                                try:
                                    ...
                                    control.log('Head Request failed link likely dead, skipping')
                                    continue
                            ...
                            try:
                                ...
                                headers = control.quote_plus(headers).replace('%3D', '=') if ' ' in headers else headers
                                headers = dict(control.parse_qsl(headers))
                        ...
                        elif control.file_exists(i['url']):
                            stream_link = i['url']
    ...
    def getHosterList(self):
        ...
        try:
            ...
            try:
                if control.getSetting('premiumize.enabled') == 'true' and control.getSetting(
            ...
            try:
                if control.getSetting('realdebrid.enabled') == 'true' and control.getSetting('rd.hosters') == 'true':
                    thread_pool.put(real_debrid.RealDebrid().get_hosters, hosters)
            except:
                pass

            try:
                if control.getSetting('alldebrid.enabled') == 'true' and control.getSetting('alldebrid.hosters') == 'true':
    ...
    def doModal(self, sources, args, pack_select):
        ...
        self.setProperty('release_title', control.display_string(self.sources[0]['release_title']))
