...
class judaeaPlayer(control.player):
    def __init__(self):
        ...
        self.scrobbling_enabled = control.getSetting('trakt.scrobbling') == 'true'
        self.scrobbled = False
        self.original_action_args = ''
        self.smart_playlists = control.getSetting('smartplay.playlistcreate')
        self.smart_module = None
        self.ignoreSecondsAtStart = int(control.get_advanced_setting('video', 'ignoresecondsatstart'))
        self.ignorePercentAtEnd = int(control.get_advanced_setting('video', 'ignorepercentatend'))
        self.playCountMinimumPercent = int(control.get_advanced_setting('video', 'playcountminimumpercent'))

        self.min_time_before_scrape = int(control.getSetting('playingnext.time')) + \
                                      int(int(control.getSetting('general.timeout'))) + 20
        self.marked_watched = False
        control.player.__init__(self)
    ...
    def play_source(self, stream_link, args, resume_time=None, params=None):
        try:
            ...
            orginalArgs = json.loads(control.unquote(self.original_action_args))
            ...
            item = control.menuItem(path=stream_link)
            args['info']['FileNameAndPath'] = control.unquote(stream_link)
            ...
            control.resolvedUrl(syshandle, True, item)
    ...
    def start_playback(self):
        try:
            ...
            if control.playList.size() == 1 and self.smart_playlists == 'true' and self.media_type == 'episode':
                self.smart_module.build_playlist(params=self.params)

            if control.playList.size() == control.playList.getposition() + 1 \
    ...
    def traktStopWatching(self, override_progress=None):
        ...
        if self.scrobbling_enabled and not self.scrobbled:
            ...
            if int(self.getWatchedPercent()) >= int(control.get_advanced_setting('video', 'playcountminimumpercent')):
                if self.media_type == 'episode':
                    from resources.lib.modules.trakt_sync.shows import TraktSyncDatabase
                    TraktSyncDatabase().mark_episode_watched_by_id(self.trakt_id)
                else:
    ...
    def keepAlive(self):
        ...
        if self.offset is not None and self.offset != 0:
            control.log("Seeking {} seconds".format(self.offset))
            ...
            try:
                ...
                if watched_percentage > 80 or time_left <= self.min_time_before_scrape:
                    ...
                    if self.pre_cache_initiated is False:
                        ...
                        try:
                            if control.getSetting('smartPlay.preScrape') == 'true':
        ...
        self.traktStopWatching()
        control.execute('RunPlugin("plugin://plugin.video.judaeaTEST/?action=runPlayerDialogs")')
...
class PlayerDialogs(control.player):

    def __init__(self):
        super(PlayerDialogs, self).__init__()
        self._min_time = int(control.getSetting('playingnext.time'))
        self.playing_file = self.getPlayingFile()

    def display_dialog(self):

        if control.playList.size() > 0 and control.playList.getposition() != (control.playList.size() - 1):
            if control.getSetting('smartplay.stillwatching') == 'true' and self._still_watching_calc():

                target = self._show_still_watching

            elif control.getSetting('smartplay.playingnextdialog') == 'true':
    ...
    def _still_watching_calc():
        calculation = float(control.playList.getposition() + 1) / \
                      float(control.getSetting('stillwatching.numepisodes'))
    ...
    def _show_playing_next(self):
        ...
        from resources.lib.modules.windows.playing_next import PlayingNext
    ...
    def _show_still_watching(self):
        ...
        from resources.lib.modules.windows.still_watching import StillWatching
        from resources.lib.modules.skin_manager import SkinManager
    ...
    def _get_next_item_args():
        ...
        params = dict(control.parse_qsl(url.replace('?', '')))
    ...
    def _is_video_window_open():
        ...
        if control.kodiGui.getCurrentWindowId() != 12005:
            return False
        return True
