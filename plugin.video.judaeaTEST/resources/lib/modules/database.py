...
def get(function, duration, *args, **kwargs):
    ...
    try:
        sources = False
        reload = False
        if 'judaea_reload' in kwargs:
            reload = kwargs['judaea_reload']
            kwargs.pop('judaea_reload')

        if 'judaea_sources' in kwargs:
            sources = True
            kwargs.pop('judaea_sources')
    ...
    try:
        ...
        for t in [cache_table, 'rel_list', 'rel_lib']:
            try:
                ...
                pass
        control.showDialog.notification('{}: {}'.format(control.addonName, control.lang(40306)), control.lang(32078), time=5000)
    except:
        pass
    finally:
        control.try_release_lock(control.cacheFile_lock)
...
def clearSearchHistory():
    try:
        control.searchHistoryDB_lock.acquire()
        confirmation = control.showDialog.yesno(control.addonName, control.lang(40169))
        if not confirmation:
            return
        cursor = _get_connection_cursor(control.searchHistoryDB)
        ...
        cursor.connection.commit()
        cursor.close()
        control.showDialog.notification(control.addonName, control.lang(40259), time=5000)
    except:
        try:
            cursor.close()
        except:
...
def getTorrents(item_meta):
    if control.getSetting('general.torrentCache') == 'false':
        return []
...
def torrent_cache_clear():
    confirmation = control.showDialog.yesno(control.addonName, control.lang(32043))
    if not confirmation:
        return
    try:
        control.torrentScrapeCacheFile_lock.acquire()
    ...
    control.showDialog.notification('{}: {}'.format(control.addonName, control.lang(40306)), control.lang(32079), time=5000)


def get_assist_torrents():
    try:
        control.activeTorrentsDBFile_lock.acquire()
        cursor = _get_connection_cursor(control.activeTorrentsDBFile)
        ...
        control.try_release_lock(control.activeTorrentsDBFile_lock)
...
def clear_assist_torrents():
    try:
        ...
        control.try_release_lock(control.activeTorrentsDBFile_lock)
    control.showDialog.notification('{}: {}'.format(control.addonName, control.lang(40306)), control.lang(32080), time=5000)


def get_single_provider(provider_name, package, country):
    try:
        control.providersDB_lock.acquire()
        cursor = _get_connection_cursor(control.providersDB)
        ...
        control.try_release_lock(control.providersDB_lock)
...
def get_provider_packages():
    try:
        control.log('Getting provider packages')
        control.providersDB_lock.acquire()
        cursor = _get_connection_cursor(control.providersDB)
...
def get_premiumize_transfers():
    try:
        control.premiumizeDB_lock.acquire()
        cursor = _get_connection_cursor(control.premiumizeDB)
        cursor.execute(
        ...
        control.try_release_lock(control.premiumizeDB_lock)
...
def _find_cache_version():
    ...
    try:
        ...
        oldVersion = '0'
    try:
        curVersion = control.addon('script.module.judaea').getAddonInfo('version')
...
def clear_local_bookmarks():
    cursor = _get_connection_cursor(control.get_video_database_path())
    cursor.execute("SELECT * FROM files WHERE strFilename LIKE '%plugin.video.judaeaTEST%'")
