...
class Cloudflare:
    def __init__(self, response):
        self.timeout = 5
        self.domain = control.urlparse(response["url"])[1]
        self.protocol = control.urlparse(response["url"])[0]
        self.js_data = {}
        self.header_data = {}
    ...
    def get_url(self):
        ...
        if self.js_data.get("wait", 0):
            ...
            response = "%s://%s%s?%s" % (
                self.protocol, self.domain, self.js_data["auth_url"], control.urlencode(self.js_data["params"]))
        ...
        if self.header_data.get("wait", 0):
            response = "%s://%s%s?%s" % (
                self.protocol, self.domain, self.header_data["auth_url"], control.urlencode(self.header_data["params"]))
