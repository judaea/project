...
class SkinNotFoundException(Exception):
    ...
    def __init__(self, skin_name):
        control.log('Unable to find skin "{}", check it\'s installed?'.format(skin_name), 'error')


class SkinUpdateFailed(Exception):

    def __init__(self, msg):
        control.log('Skin Update Failed: \n {}'.format(msg), 'error')
...
class SkinManager:
    ...
    def __init__(self):
        ...
        self.judaea_skins = ['Judaea', 'Default']

        if not os.path.exists(control.SKINS_PATH):
            os.mkdir(control.SKINS_PATH)
    ...
    def get_active_skin_path(self):
        ...
        if active_skin_name in self.judaea_skins:
            return control.ADDON_PATH
        else:
            return os.path.join(control.dataPath, 'skins', active_skin_name)
    ...
    def get_active_skin(self):
        ...
        if active_skin is None:
            control.log('Failed to identify active skin, resetting to Default', 'error')
            cursor.execute('UPDATE skins SET active=1 WHERE skin_name == ?', ('Judaea',))
    ...
    def install_skin(self, zip_location=None, silent=False):
        ...
        if skin_meta['skin_name'] in self.judaea_skins:
            control.showDialog.ok(control.addonName, control.lang(40168) % skin_meta['skin_name'])
            return
        ...
        if not silent:
            switch_skin = control.showDialog.yesno(control.addonName, control.lang(40158) %
    ...
    def uninstall_skin(self, skin_name=None):
        ...
        confirmation = control.showDialog.yesno(control.addonName, control.lang(40159) % skin_name)

        if not confirmation:
            return

        if self._is_skin_active(skin_name):
            confirmation = control.showDialog.yesno(control.addonName, control.lang(40166), nolabel='Cancel', yeslabel="Ok")
            if not confirmation:
                return
            self.switch_skin('Judaea')

        skin_path = os.path.join(control.SKINS_PATH, skin_name)
        ...
        control.showDialog.ok(control.addonName, control.lang(40160) % skin_name)
    ...
    def switch_skin(self, skin_name=None):
        ...
        control.showDialog.ok(control.addonName, control.lang(40167) % skin_name)
    ...
    def _select_installed_skin(self, hide_default=False):
        ...
        if hide_default:
            installed_skins.remove([i for i in installed_skins if i[1] == 'Judaea'][0])

        selection = control.showDialog.select(control.addonName, [i[0] for i in installed_skins])
    ...
    def _extract_zip(self, install_package, skin_meta):
        ...
        if '%sresources/' % file_path not in install_package.namelist():
            control.log('Theme Folder Structure Invalid: Missing folder "Resources"')
            control.showDialog.ok(control.addonName, control.lang(40171))
            raise Exception

        skin_path = os.path.join(control.SKINS_PATH, skin_meta['skin_name'])
    ...
    def _get_zip_file(self, zip_location, silent=False):
        ...
        if zip_location.startswith('smb'):
            if not silent:
                control.showDialog.ok(control.addonName, control.lang(33014))
            return

        if zip_location.startswith('http'):
            response = requests.get(zip_location, stream=True)
            if not response.ok and not silent:
                control.showDialog.ok(control.addonName, control.lang(33015))
                return
            else:
                pass
            try:
    ...
    def _get_zip_location(self):
        ...
        install_type = control.showDialog.select(control.addonName, [control.lang(40302), control.lang(40303)])
        ...
        if install_type == 0:
            ...
            user_input = control.showKeyboard('', '%s: %s' % (control.addonName, control.lang(40305)))
            user_input.doModal()
    ...
    def _mark_skin_active(self, skin_name):
        ...
        control.setSetting('skin.active', skin_name)

    def _add_skin_to_database(self, skin_meta):

        if skin_meta['skin_name'] in self.judaea_skins:
            control.showDialog.ok(control.addonName, control.lang(40321))
            return
    ...
    def confirm_skin_path(self, xml_file):
        ...
        if self.active_skin_path == control.ADDON_PATH:
        ...
        return xml_file, control.ADDON_PATH

    def _check_skin_for_update(self, skin_info):

        try:
            remote_meta = requests.get(skin_info['remote_meta']).json()
            return control.check_version_numbers(skin_info['version'], remote_meta['version'])
        except:
            control.log('Failed to obtain remote meta information for skin: {}'.format(skin_info['skin_name']))
            return False
    ...
    def check_for_updates(self, skin_name=None, silent=False):
        ...
        if not silent:
            control.progressDialog.create(control.addonName, control.lang(33019))
        ...
        if len(skins) == 0:
            if not silent:
                ...
                control.showDialog.ok(control.addonName, control.lang(33018))
            return

        if not silent:
            control.progressDialog.close()
            while skins and len(skins) > 0:
                control.progressDialog.create(control.addonName, control.lang(40311))
                control.progressDialog.update(-1)

                selection = control.showDialog.select(control.addonName, ['{} - {}'.format(i['skin_name'], i['version'])
                                                                      for i in skins])
                if selection == -1:
                    return
                ...
                try:
                    ...
                    control.progressDialog.close()
                    control.showDialog.ok(control.addonName, control.lang(40318))
                except:
                    import traceback
                    traceback.print_exc()
                    control.log('Failed to update skin: {}'.format(selection['skin_name']))
                    control.showDialog.notification(control.addonName, control.lang(33013))
        ...
        for skin in skins:
            try:
                ...
                control.log('Failed to update theme: {}'.format(skin['skin_name']))

        control.log('Skin updates completed')

    def _get_skin_cursor(self):
        return database._get_connection_cursor(control.SKINS_DB_PATH)

    def _check_database_for_updates(self):
        cursor = self._get_skin_cursor()
        try:
            cursor.execute("SELECT * FROM skins WHERE skin_name=?", ('Judaea',))
