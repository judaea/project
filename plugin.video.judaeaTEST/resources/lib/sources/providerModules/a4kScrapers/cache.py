...
def __get_cache_core(query):
    if __cache_results.get(query, '') != '':
        if CACHE_LOG:
            control.log('get_cache_local: %s' % query, 'notice')
        return __cache_results[query]
    else:
        __cache_results[query] = {}

    if CACHE_LOG:
        control.log('get_cache_request: %s' % query, 'notice')

    response = __dynamo_get(__map_in_cache(query))

    if response.status_code != 200:
        if CACHE_LOG:
            control.log('get_cache_err_response: %s' % query, 'notice')
        return __cache_results[query]

    result = __map_out_cache(response.text)
    if result is None:
        if CACHE_LOG:
            control.log('get_cache_nocache: %s' % query, 'notice')
        return __cache_results[query]
    ...
    if CACHE_LOG:
        control.log('get_cache_result: %s' % query, 'notice')
...
def __set_cache_core(query, cached_results):
    try:
        ...
        if CACHE_LOG:
            control.log('set_cache_request: %s' % query, 'notice')

        response = __dynamo_put(__map_in_cache(item))
        if response.status_code >= 400:
          if CACHE_LOG:
            control.log('set_cache_request_err: %s, status_code=%s, text=%s' % (query, response.status_code, response.text), 'notice')
    except:
        traceback.print_exc()
