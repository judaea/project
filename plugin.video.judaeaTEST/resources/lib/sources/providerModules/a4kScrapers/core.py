...
class DefaultSources(object):
    ...
    def cancel(self):
        ...
        control.log('a4kScrapers.%s.%s cancellation requested' % (self.query_type, self._caller_name), 'notice')
...
class DefaultHosterSources(DefaultSources):
    ...
    def sources(self, simple_info, hostDict, hostprDict):
        ...
        try:
            ...
            for result in hoster_results:
                ...
                for url in result.urls:
                    ...
                    if DEV_MODE and len(sources) == 0:
                        control.log(release_title, 'info')
                    sources.append({
            ...
            control.log('a4kScrapers.%s.%s: %s' % (self.query_type, self._caller_name, result_count), 'notice')
...
class CoreScraper(object):
    ...
    def _sanitize_and_get_status(self):
        ...
        for torrent in self._results:
            ...
            if DEV_MODE:
                control.log(torrent['release_title'], 'notice')
    ...
    def _get_movie_results(self):
        control.log('a4kScrapers.movie.%s: %s' % (self.caller_name, self._sanitize_and_get_status()), 'notice')
        return self._results

    def _get_episode_results(self):
        control.log('a4kScrapers.episode.%s: %s' % (self.caller_name, self._sanitize_and_get_status()), 'notice')
        return self._results
