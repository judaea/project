...
class MetaCache():
    def __init__(self):
        self.datapath = xbmc.translatePath("special://userdata/addon_data/script.module.davidmeta")
    ...
    def get_memory_cache(self, db_type, id_type, media_id, current_time):
        ...
        try:
            if db_type in ('movie', 'tvshow'):
                ...
                string = 'davidmeta_season_%s' % str(media_id)
    ...
    def set_memory_cache(self, db_type, meta, expires, tmdb_id):
        if db_type in ('movie', 'tvshow'):
            ...
            string = 'davidmeta_season_%s' % str(tmdb_id)
            cachedata = (expires, meta)
    ...
    def delete_memory_cache(self, db_type, id_type, media_id):
        ...
        else: string = 'davidmeta_season_%s' % media_id
    ...
    def delete_all(self):
        try:
            ...
            for i in all_entries: self.delete_memory_cache(i[0], 'tmdb_id', i[1])
            dbcur.execute("SELECT tmdb_id FROM season_metadata")
            all_season_entries = dbcur.fetchall()
            for i in all_season_entries: self.delete_memory_cache('season', 'tmdb_id', i[0])
            dbcur.execute("SELECT string_id FROM function_cache")
            function_cache_entries = dbcur.fetchall()
            for i in function_cache_entries: self.delete_function_cache(i[0])
    ...
    def check_cleanup(self):
        while window.getProperty("davidmetacachecleanbusy") == 'busy':
            xbmc.sleep(25)
        cur_time = self.time
        lastexecuted = window.getProperty("davidmeta.clean.lastexecuted")
        if not lastexecuted:
            self._do_cleanup()
    ...
    def _do_cleanup(self):
        ...
        xbmc.log('[DAVIDMETA]: Running Database Clean...', 2)
        if window.getProperty("davidmetacachecleanbusy"):
            return
        window.setProperty("davidmetacachecleanbusy", "busy")
        dbcon = database.connect(self.dbfile)
        dbcur = dbcon.cursor()
        ...
        dbcon.execute("VACUUM")
        dbcon.close()
        window.setProperty("davidmeta.clean.lastexecuted", repr(cur_time))
        window.clearProperty("davidmetacachecleanbusy")
        xbmc.log('[DAVIDMETA]: Database Clean Finished', 2)
...
def cache_function(function, string, url, expiration=96, json=True):
    from davidmeta.utils import to_utf8
