...
__addon__ = xbmcaddon.Addon(id='script.module.davidmeta')
...
def notification(line1, time=5000, icon=__addon__.getAddonInfo('icon'), sound=False):
    import xbmcgui
    xbmcgui.Dialog().notification('David Meta', line1, icon, time, sound)
...
def user_info():
    ...
    return {'tmdb_api': tmdb_api, 'tvdb_api': tvdb_api, 'tvdb_jwtoken': tvdb_jwtoken, 'extra_fanart_enabled': extra_fanart_enabled, 'gif_posters_enabled': gif_posters_enabled,
            'fanart_client_key': fanart_client_key, 'kyra_api_key': kyra_api_key, 'kyra_user_key': kyra_user_key, 'language': meta_language}
...
def choose_language():
    ...
    list_choose = dialog.select('Davidmeta - Choose Meta Language', [i['name'] for i in langs])
    if list_choose >= 0:
...
def open_settings(query):
    try:
        ...
        xbmc.executebuiltin('Addon.OpenSettings(script.module.davidmeta)')
