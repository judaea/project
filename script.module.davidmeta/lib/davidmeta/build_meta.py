...
import os
import json
from davidmeta import tmdb
from davidmeta import tvdb
from davidmeta import trakt
from davidmeta import fanarttv
from davidmeta.kyradb import add as kiradb_add
from davidmeta.kyradb import KyraDBAPI as kyradb
from davidmeta.metacache import MetaCache
from davidmeta.utils import try_parse_int, safe_string, remove_accents, to_utf8
# from davidmeta.utils import logger

__addon__ = xbmcaddon.Addon("script.module.davidmeta")
...
def getMovieMeta(id_type, media_id, user_info, hours=720):
    ...
    def fanarttv_meta(fanart_id, fetch_fanart_art):
        if extra_fanart_enabled or fetch_fanart_art: return fanarttv.get('movies', language, fanart_id, user_info['fanart_client_key'])
        else: return None
    ...
    if not meta:
        try:
            ...
            if not tmdb_data.get('poster_path'):
                fetch_fanart_art = True
            fanarttv_data = fanarttv_meta(tmdb_data['id'], fetch_fanart_art)
            if fetch_fanart_art:
...
def getAllEpisodes(media_id, tvdb_id, seasons, tmdb_data, user_info, hours=96):
    ...
    def _return_meta(data, use_tmdb=False):
        meta = buildSeasonsMeta(data, seasons, tmdb_data, use_tmdb=use_tmdb)
...
def getSeasonEpisodes(media_id, tvdb_id, season, seasons, tmdb_data, user_info, all_episodes=False, hours=96):
    ...
    if data:
        ...
        episodes_data = buildEpisodesMeta(episodes_data, use_tmdb=use_tmdb)
...
def buildMeta(db_type, data, fanarttv_data=None, kyradb_data=None, tvdb_summary=None):
    ...
    if data.get('poster_path'):
        meta['poster'] = "http://image.tmdb.org/t/p/original%s" % data.get('poster_path')
    elif data.get('external_poster'):
        meta['poster'] = data['external_poster']
    else:
        meta['poster'] = xbmc.translatePath(os.path.join(addon_dir, "resources", "default_images", "davidmeta_blank_poster.png"))
    if data.get('backdrop_path'):
        meta['fanart'] = "http://image.tmdb.org/t/p/original%s" % data.get('backdrop_path')
    elif data.get('external_fanart'):
        meta['fanart'] = data['external_fanart']
    else:
        meta['fanart'] = xbmc.translatePath(os.path.join(addon_dir, "resources", "default_images", "davidmeta_blank_fanart.png"))
    if fanarttv_data:
        ...
        meta['banner'] = xbmc.translatePath(os.path.join(addon_dir, "resources", "default_images", "davidmeta_blank_banner.png"))
        meta['clearart'] = ''
        meta['clearlogo'] = ''
        meta['landscape'] = ''
        meta['discart'] = ''
...
def buildSeasonsMeta(data, seasons, tmdb_data, use_tmdb=False):
    ...
    for i in seasons:
        ...
        try: season_info['poster_path'] = ["http://image.tmdb.org/t/p/original%s" % p['poster_path'] for p in tmdb_data if p['season_number'] == i and p['poster_path'] is not None][0]
        except: pass
...
def buildEpisodesMeta(data, use_tmdb=False):
    meta = []
    if use_tmdb:
        for i in data:
            episode_info = {}
            ...
            episode_info['thumb'] = 'http://image.tmdb.org/t/p/original%s' % i['still_path'] if i.get('still_path', None) is not None else None
