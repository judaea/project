...
__addon_id__ = 'script.module.davidmeta'
...
class TvdbAPI:
    ...
    def pause_token_renew(self):
        if not window.getProperty('davidmeta.fetching.tvdb.jwtoken') == 'true':
            return False
        for i in range(0, 20):
            if window.getProperty('davidmeta.fetching.tvdb.jwtoken') == 'true':
                time.sleep(1)
            else:
                self.jw_token = window.getProperty('davidmeta.new.tvdb.jwtoken')
                return True

    def pause_token_new(self):
        if not window.getProperty('davidmeta.fetching.new.tvdb.jwtoken') == 'true':
            return False
        for i in range(0, 20):
            if window.getProperty('davidmeta.fetching.new.tvdb.jwtoken') == 'true':
                time.sleep(1)
            else:
                self.jw_token = window.getProperty('davidmeta.new.tvdb.jwtoken')
                return True
    ...
    def get_new_jwtoken(self):
        ...
        if not pause_token_new:
            window.setProperty('davidmeta.fetching.new.tvdb.jwtoken', 'true')
            window.clearProperty('davidmeta.new.tvdb.jwtoken')
        else: return
        ...
        window.setProperty('davidmeta.new.tvdb.jwtoken', self.jw_token)
        window.clearProperty('davidmeta.fetching.new.tvdb.jwtoken')

    def renew_jwtoken(self):
        pause_token_renew = self.pause_token_renew()
        if not pause_token_renew:
            window.setProperty('davidmeta.fetching.tvdb.jwtoken', 'true')
            window.clearProperty('davidmeta.new.tvdb.jwtoken')
        else: return
        ...
        if 'Error' in response:
            window.clearProperty('davidmeta.fetching.tvdb.jwtoken')
