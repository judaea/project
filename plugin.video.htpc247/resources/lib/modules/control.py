...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.htpctv.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.htpctv.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.htpctv.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.htpctv.artwork)')
...
def moderator():
    netloc = [urlparse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.live.streamspro', 'plugin.video.phstreams', 'plugin.video.cpstreams', 'plugin.video.streamarmy', 'plugin.video.tinklepad', 'plugin.video.metalliq']

    if not infoLabel('Container.PluginName') in netloc: sys.exit()


def metaFile():
    if condVisibility('System.HasAddon(script.htpctv.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.htpctv.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
