...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2p1ZGFlYS9wcm9qZWN0L21hc3Rlci9tZW51cy8yNDcvaG9tZS54bWw=')
            self.list = self.htpctv_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.htpctv_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.htpctv_list('', result=result)
    ...
    def search(self, url):
        try:
            mark = False
            if (url == None or url == ''):
                self.list = [{'name': 32702, 'action': 'addSearch'}]
                self.list += [{'name': 32703, 'action': 'delSearch'}]
            else:
                if '|SECTION|' in url: mark = url.split('|SECTION|')[0]
                self.list = [{'name': 32702, 'url': url, 'action': 'addSearch'}]
                self.list += [{'name': 32703, 'action': 'delSearch'}]
    ...
    def addSearch(self, url):
            ...
            link = 'http://cosmix.offshorepastebin.com/bennu/search.xml'

            if skip == 0:
                if section == 1:
                    keyboard = control.keyboard('', control.lang(32702).encode('utf-8'))
                    keyboard.doModal()
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.htpctv_list, link))
    ...
    def htpctv_list(self, url, result=None):
            ...
            try:
                ...
                if build:
                    ...
                    if content in ['movies', 'tvshows']:
                        ...
                        cm.append((control.lang(32707).encode('utf-8'), 'RunPlugin(%s?action=trailer&name=%s)' % (sysaddon, urllib.quote_plus(name))))

                    if content in ['movies', 'tvshows', 'seasons', 'episodes']:
                        cm.append((control.lang(32708).encode('utf-8'), 'XBMC.Action(Info)'))

                    if (folder == False and not '|regex=' in str(i.get('url'))) or (folder == True and content in ['tvshows', 'seasons']):
                        cm.append((control.lang(32723).encode('utf-8'), 'RunPlugin(%s?action=queueItem)' % sysaddon))

                    if content == 'movies':
                        try: dfile = '%s (%s)' % (i['title'], i['year'])
                        except: dfile = name
                        try: cm.append((control.lang(32722).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (sysaddon, urllib.quote_plus(dfile), urllib.quote_plus(i['url']), urllib.quote_plus(poster))))
                        except: pass
                        ...
                        try: cm.append((control.lang(32722).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (sysaddon, urllib.quote_plus(name), urllib.quote_plus(i['url']), urllib.quote_plus(poster))))
                        except: pass

                    if mode == 'movies':
                        cm.append((control.lang(32711).encode('utf-8'), 'RunPlugin(%s?action=addView&content=movies)' % sysaddon))
                    elif mode == 'tvshows':
                        cm.append((control.lang(32712).encode('utf-8'), 'RunPlugin(%s?action=addView&content=tvshows)' % sysaddon))
                    elif mode == 'seasons':
                        cm.append((control.lang(32713).encode('utf-8'), 'RunPlugin(%s?action=addView&content=seasons)' % sysaddon))
                    elif mode == 'episodes':
                        cm.append((control.lang(32714).encode('utf-8'), 'RunPlugin(%s?action=addView&content=episodes)' % sysaddon))
...
class resolver:
    ...
    def link(self, url):
        try:
            ...
            if url == None: return control.infoDialog(control.lang(32705).encode('utf-8'))
            return url
        except:
            pass
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            u = sources2.sources2().getSources(title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality)
        ...
        try:
            ...
            u = sources2.sources2().getURISource(url)
