...
class sources2:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=10):
        u = None
        ...
        for i in range(0, (timeout * 2) + 60):
            try:
                ...
                try:
                    ...
                    string4 = "htmc247 is searching links for: [COLOR yellow][B]%s (%s)[/COLOR][/B] - Time Elapsed: [COLOR ghostwhite]%s Seconds[/COLOR]"  % (title,year,str(timerange))
        ...
        progressDialog.update(100, control.lang(32726).encode('utf-8'), control.lang(32731).encode('utf-8'))
    ...
    def getURISource(self, url):
        ...
        progressDialog = control.progressDialog
        progressDialog.create(control.addonInfo('name'), control.lang(32726).encode('utf-8'))
        progressDialog.update(0)

        progressDialog.update(0, control.lang(32726).encode('utf-8'), control.lang(32731).encode('utf-8'))

        self.sources = call.sources(url, self.hostDict, self.hostprDict)
        for i in self.sources: i.update({'provider': source})

        progressDialog.update(50, control.lang(32726).encode('utf-8'), control.lang(32731).encode('utf-8'))
