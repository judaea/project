...
def backUpOptions(type, name=""):
	...
	#exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore']
	exclude_dirs  = ['cache', 'system', 'Thumbnails', 'peripheral_data', 'temp', 'My_Builds', 'library']
	exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore', 'XBMCHelper.conf']
	bad_files     = [
					(os.path.join(ADDOND, 'plugin.video.htpctv', 'cache.db')),
					(os.path.join(ADDOND, 'plugin.video.htpctv', 'cache.meta.5.db')),
					(os.path.join(ADDOND, 'plugin.video.htpctv', 'cache.providers.13.db')),
...
def RESTOREFAV():
	if os.path.exists(FAVFILE):
		choice = xbmcgui.Dialog().yesno(ADDONTITLE, 'Do you want to Restore your Favorites?', '', '', yeslabel='[COLOR=green]Yes[/COLOR]',nolabel='[COLOR=red]No[/COLOR]')
		if choice == 0:
			return
		elif choice == 1:
	...
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]No Back-up found![/COLOR]' % COLOR2)
			
def BACKUPFAV():
	if not os.path.exists(FAVDEST):  os.makedirs(FAVDEST)
	if os.path.exists(FAVOURITES):
		choice = xbmcgui.Dialog().yesno(ADDONTITLE, 'Do you want to Back-up your Favorites?', '', '', yeslabel='[COLOR=green]Yes[/COLOR]',nolabel='[COLOR=red]No[/COLOR]')
		if choice == 0:
			return
		elif choice == 1:
...
def DELFAV():
	if os.path.exists(FAVFILE):
		choice = xbmcgui.Dialog().yesno(ADDONTITLE, 'Are you sure you want to PERMANENTLY delete your Favorites Back-up?!?!', '', '', yeslabel='[COLOR=green]Yes[/COLOR]',nolabel='[COLOR=red]No[/COLOR]')
		if choice == 0:
			return
		elif choice == 1:
			shutil.rmtree(os.path.join(FAVDEST))
			DIALOG.ok(ADDONTITLE,'[COLOR=red]COMPLETE[/COLOR]', 'Back-up deleted.', '')
