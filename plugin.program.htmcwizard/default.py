...
SAVEDATA            = wiz.getS('savedata')
BACKUP              = wiz.getS('backup')
WIPE                = wiz.getS('wipe')
...
ICONDEV             = uservar.ICONDEV if not uservar.ICONDEV == 'http://' else ICON
ICONFRESH           = uservar.ICONFRESH if not uservar.ICONFRESH == 'http://' else ICON
ICONKODI            = uservar.ICONKODI if not uservar.ICONKODI == 'http://' else ICON
...
ICONTOOLS           = uservar.ICONTOOLS if not uservar.ICONTOOLS == 'http://' else ICON
ICONTRAKT           = uservar.ICONTRAKT if not uservar.ICONTRAKT == 'http://' else ICON
ICONWIPE            = uservar.ICONWIPE if not uservar.ICONWIPE == 'http://' else ICON
ICONWIZARD          = uservar.ICONWIZARD if not uservar.ICONWIZARD == 'http://' else ICON
...
def index():
    ...
    addDir ('Builds', 'builds',   icon=ICONBUILDS,   themeit=THEME3)
    addDir ('Maintenance', 'maint', icon=ICONMAINT, themeit=THEME6)
    if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('Apk Installer' ,'apk', icon=ICONAPK, themeit=THEME3)
    ...
    if SAVEDATA == 'true': addDir('Save Data Menu', 'savedata', icon=ICONSAVE, themeit=THEME2)
    if BACKUP == 'true': addDir('Backup/Restore Menu', 'backup', icon=ICONBACKUP, themeit=THEME2)
    if WIPE == 'true': addDir('Wipe Device Menu', 'wipe', icon=ICONWIPE, themeit=THEME7)
    if DEVELOPER == 'true': addDir('Developer Menu', 'developer', icon=ICONDEV, themeit=THEME2)
    addFile('Tools', 'settings', icon=ICONSETTINGS, themeit=THEME1)
    addFile('View Errors in Log: %s' % (errorsfound), 'viewerrorlog', icon=ICONMAINT, themeit=THEME6)
    if errors > 0: addFile('View Last Error In Log', 'viewerrorlast', icon=ICONMAINT, themeit=THEME6)
...
def buildMenu():
    ...
    if third == True:
        for item in addin:
            ...
            addDir ("[B]%s[/B]" % name, 'viewthirdparty', item, icon=ICONTOOLS, themeit=THEME3)
    if len(match) >= 1:
        if SEPERATE == 'true':
            for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
...
def viewThirdList(number):
    ...
    if not work == True:
        addFile('Url for txt file not valid', '', icon=ICONTOOLS, themeit=THEME3)
        addFile('%s' % WORKINGURL, '', icon=ICONTOOLS, themeit=THEME3)
    else:
...
def maintMenu(view=None):
    ...
    if view == "clean" or SHOWMAINT == 'true':
        addFile('Total Clean Up: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize),    'fullclean',       icon=ICONMAINT, themeit=THEME6)
        addFile('Clear Cache: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache),      'clearcache',      icon=ICONMAINT, themeit=THEME6)
        addFile('Clear Packages: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack),  'clearpackages',   icon=ICONMAINT, themeit=THEME6)
        addFile('Clear Thumbnails: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),  'clearthumb',     icon=ICONMAINT, themeit=THEME6)
        if os.path.exists(ARCHIVE_CACHE): addFile('Clear Archive_Cache: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(archive), 'cleararchive',   icon=ICONMAINT, themeit=THEME6)
        addFile('Clear Old Thumbnails', 'oldThumbs',      icon=ICONMAINT, themeit=THEME6)

    addDir ('[B]Misc Maintenance[/B]'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME2)
    if view == "misc" or SHOWMAINT == 'true': 
        if errors > 0: addFile('View Last Error In Log', 'viewerrorlast', icon=ICONTOOLS, themeit=THEME6)
        addFile('View Errors in Log: %s' % (errorsfound), 'viewerrorlog', icon=ICONTOOLS, themeit=THEME6)
        addFile('View Log File',                  'viewlog',         icon=ICONTOOLS, themeit=THEME6)
        addFile('Force Update Addons',            'forceupdate',     icon=ICONTOOLS, themeit=THEME6)
        addFile('Kodi 17 Fix - Enable Addons',    'kodi17fix',       icon=ICONTOOLS, themeit=THEME6)
        addFile('Kodi 17 Fix - Enable Unknown Sources', 'unknownsources',  icon=ICONTOOLS, themeit=THEME6)
        addFile('Reload Skin',                    'forceskin',       icon=ICONTOOLS, themeit=THEME6)
        addFile('Reload Profile',                 'forceprofile',    icon=ICONTOOLS, themeit=THEME6)
        addFile('Upload Kodi.log',                'uploadlog',       icon=ICONTOOLS, themeit=THEME6)
        addFile('View Wizard Log File',           'viewwizlog',      icon=ICONTOOLS, themeit=THEME6)
        addFile('Clear Wizard Log File%s' % wizlogsize,'clearwizlog',     icon=ICONTOOLS, themeit=THEME6)
        addFile('Clear Crash Logs',               'clearcrash',      icon=ICONTOOLS, themeit=THEME6)
        addFile('Force Close Kodi',               'forceclose',      icon=ICONTOOLS, themeit=THEME6)

    addDir ('[B]System Tweaks/Fixes[/B]',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME2)
    if view == "tweaks" or SHOWMAINT == 'true': 
        addFile('Scan Sources for broken links',    'checksources',     icon=ICONTOOLS, themeit=THEME6)
        addFile('Scan For Broken Repositories',     'checkrepos',       icon=ICONTOOLS, themeit=THEME6)
        addFile('Force Update Text Files',          'forcetext',        icon=ICONTOOLS, themeit=THEME6)
        addFile('Remove Non-Ascii filenames',       'asciicheck',       icon=ICONTOOLS, themeit=THEME6)
        addFile('Convert Paths to special',         'convertpath',      icon=ICONTOOLS, themeit=THEME6)
        addFile('Hide Passwords On Keyboard Entry',   'hidepassword',   icon=ICONTOOLS, themeit=THEME6)
        addFile('Unhide Passwords On Keyboard Entry', 'unhidepassword', icon=ICONTOOLS, themeit=THEME6)
        addFile('Fix Addons Not Updating (Kodi will Force Close...)',  'fixaddonupdate',  icon=ICONTOOLS, themeit=THEME6)
        addDir ('System Information',             'systeminfo',         icon=ICONTOOLS, themeit=THEME6)

    addDir ('[B]Speed Test[/B]',            'maint', 'speedtest',       icon=ICONMAINT, themeit=THEME2)
    if view == "speedtest" or SHOWMAINT == 'true':
        addDir ('Run Speed Tester' ,'speedtestM', icon=ICONTOOLS, themeit=THEME6)
        addDir ('View IP Address & MAC Address',        'viewIP',    icon=ICONTOOLS, themeit=THEME6)

    addDir ('[B]Advanced Settings Tool[/B]',    'maint', 'autoconfig',          icon=ICONMAINT, themeit=THEME2)
    if view == "autoconfig" or SHOWMAINT == 'true':
        if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
            addDir ('Advanced Settings',            'advancedsetting',  icon=ICONTOOLS, themeit=THEME6)
        else: 
            if os.path.exists(ADVANCED):
                addFile('View Current AdvancedSettings.xml',    'currentsettings', icon=ICONTOOLS, themeit=THEME6)
                addFile('Remove Current AdvancedSettings.xml',  'removeadvanced',  icon=ICONTOOLS, themeit=THEME6)
            addFile('Quick Configure AdvancedSettings.xml',     'autoadvanced',    icon=ICONTOOLS, themeit=THEME6)
            addFile('Full Configure AdvancedSettings.xml',      'autoadvanced1',   icon=ICONTOOLS, themeit=THEME6)

    addFile('[B]Show All Maintenance[/B]: %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)

    addFile('[B]Third Party Wizards[/B]: %s' % thirdparty.replace('true',on).replace('false',off) ,'togglesetting', 'enable3rd', fanart=FANART, icon=ICONMAINT, themeit=THEME2)
    if thirdparty == 'true':
        ...
        addFile('Edit Third Party Wizard 1: [COLOR %s]%s[/COLOR]' % (COLOR2, first), 'editthird', '1', icon=ICONBUILDS, themeit=THEME6)
        addFile('Edit Third Party Wizard 2: [COLOR %s]%s[/COLOR]' % (COLOR2, secon), 'editthird', '2', icon=ICONBUILDS, themeit=THEME6)
        addFile('Edit Third Party Wizard 3: [COLOR %s]%s[/COLOR]' % (COLOR2, third), 'editthird', '3', icon=ICONBUILDS, themeit=THEME6)

    addFile('Auto Clean', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
    addFile('Auto Clean Up On Startup: %s' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME6)
    if autoclean == 'true':
        addFile('--- Auto Clean Fequency: [B][COLOR springgreen]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME6)
        addFile('--- Clear Cache on Startup: %s' % cache.replace('true',on).replace('false',off), 'togglesetting', 'clearcache', icon=ICONMAINT, themeit=THEME6)
        addFile('--- Clear Packages on Startup: %s' % packages.replace('true',on).replace('false',off), 'togglesetting', 'clearpackages', icon=ICONMAINT, themeit=THEME6)
        addFile('--- Clear Old Thumbs on Startup: %s' % thumbs.replace('true',on).replace('false',off), 'togglesetting', 'clearthumbs', icon=ICONMAINT, themeit=THEME6)
    addFile('Clear Video Cache', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
    addFile('Include Video Cache in Clear Cache: %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME6)
    if includevid == 'true':
        ...
        addFile('--- Include Scrubs v2: %s' % includescrubsv2.replace('true',on).replace('false',off), 'togglecache', 'includescrubsv2', icon=ICONMAINT, themeit=THEME6)
        addFile('--- Include Venom: %s' % includevenom.replace('true',on).replace('false',off), 'togglecache', 'includevenom', icon=ICONMAINT, themeit=THEME6)
        ...
        addDir ('[B][I]<< Return to Main Menu>>[/I][/B]', icon=ICON, themeit=THEME2)

    setView('files', 'viewType')

def developerMenu():
    addFile('Convert Paths to special',     'convertpath',      icon=ICONTOOLS, themeit=THEME2)
    addFile('Create QR Code',               'createqr',         icon=ICONTOOLS, themeit=THEME2)
    addFile('Test Notifications',           'testnotify',       icon=ICONTOOLS, themeit=THEME2)
    addFile('Test Update',                  'testupdate',       icon=ICONTOOLS, themeit=THEME2)
    addFile('Test First Run',               'testfirst',        icon=ICONTOOLS, themeit=THEME2)
    addFile('Test First Run Settings',      'testfirstrun',     icon=ICONTOOLS, themeit=THEME2)
    addFile('Test Auto ADV Settings',       'autoadvanced',     icon=ICONTOOLS, themeit=THEME2)
    addFile('Test APk',                     'testapk',          icon=ICONTOOLS, themeit=THEME2)
    
    setView('files', 'viewType')

def backupMenu():
    addFile('Clean Up Back Up Folder',        'clearbackup',        icon=ICONTOOLS, themeit=THEME2)
    addFile('Back Up Location: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Maintenance',  icon=ICONTOOLS, themeit=THEME2)
    addFile('[Back Up]: Build',               'backupbuild',        icon=ICONTOOLS, themeit=THEME2)
    addFile('[Back Up]: GuiFix',              'backupgui',          icon=ICONTOOLS, themeit=THEME2)
    addFile('[Back Up]: Theme',               'backuptheme',        icon=ICONTOOLS, themeit=THEME2)
    addFile('[Back Up]: Addon Pack',          'backupaddonpack',    icon=ICONTOOLS, themeit=THEME2)
    addFile('[Back Up]: Addon_data',          'backupaddon',        icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: Local Build',         'restorezip',         icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: Local GuiFix',        'restoregui',         icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: Local Addon_data',    'restoreaddon',       icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: External Build',      'restoreextzip',      icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: External GuiFix',     'restoreextgui',      icon=ICONTOOLS, themeit=THEME2)
    addFile('[Restore]: External Addon_data', 'restoreextaddon',    icon=ICONTOOLS, themeit=THEME2)
...
def saveMenu():
    ...
    addDir ('Keep Trakt Data',               'trakt',                icon=ICONTRAKT, themeit=THEME2)
    addDir ('Keep Real Debrid Data',         'realdebrid',           icon=ICONREAL,  themeit=THEME2)
    addDir ('Keep API Data',                 'login',                icon=ICONLOGIN, themeit=THEME2)
    ...
    addFile('Save Trakt Data: %s' % trakt.replace('true',on).replace('false',off)                ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME2)
    addFile('Save Real Debrid Data: %s' % real.replace('true',on).replace('false',off)           ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME2)
    addFile('Save API Data: %s' % login.replace('true',on).replace('false',off)                  ,'togglesetting', 'keeplogin',      icon=ICONLOGIN, themeit=THEME2)
    addDir ('Keep Favourites'              ,'favsmenu',    icon=ICONSAVE, themeit=THEME2)
    addFile('Keep \'Sources.xml\': %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep \'Profiles.xml\': %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep \'Advancedsettings.xml\': %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep \'Favourites.xml\': %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep Super Favourites: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep Installed Repo\'s: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME2)
    addFile('Keep My \'WhiteList\': %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME2)
    if whitelist == 'true':
        addFile('    Edit My Whitelist',        'whitelist', 'edit',   icon=ICONSAVE,  themeit=THEME2)
        addFile('    View My Whitelist',        'whitelist', 'view',   icon=ICONSAVE,  themeit=THEME2)
        addFile('    Clear My Whitelist',       'whitelist', 'clear',  icon=ICONSAVE,  themeit=THEME2)
        addFile('    Import My Whitelist',      'whitelist', 'import', icon=ICONSAVE,  themeit=THEME2)
        addFile('    Export My Whitelist',      'whitelist', 'export', icon=ICONSAVE,  themeit=THEME2)

    setView('files', 'viewType')

def wipeMenu():
    addDir ('Remove Addons',        'removeaddons',    icon=ICONTOOLS, themeit=THEME7)
    addDir ('Remove Addon Data',    'removeaddondata', icon=ICONTOOLS, themeit=THEME7)
    addDir ('Enable/Disable Addons',          'enableaddons',    icon=ICONTOOLS, themeit=THEME6)
    addFile('Enable/Disable Adult Addons',    'toggleadult',     icon=ICONTOOLS, themeit=THEME6)
    addFile('Purge Databases',      'purgedb',         icon=ICONTOOLS, themeit=THEME8)
    addFile('Fresh Start'                                   ,'freshstart',  icon=ICONFRESH, themeit=THEME8)
...
def traktMenu():
    ...
    addFile('[I]Sign up for a free Trakt account at https://trakt.tv/auth/join[/I]', '', icon=ICONTRAKT, themeit=THEME2)
    addFile('Save Trakt Data: %s' % trakt, 'togglesetting', 'keeptrakt', icon=ICONTRAKT, themeit=THEME2)
    if KEEPTRAKT == 'true': addFile('Last Save: %s' % str(last), '', icon=ICONTRAKT, themeit=THEME2)
    ...
    for trakt in traktit.ORDER:
        ...
        addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME6)
        ...
        else:                        addFile('[COLOR white]Addon Data: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
        if user == "":
        ...
        else:                        addFile('[COLOR white]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME6)
    addFile('Save All Trakt Data',          'savetrakt',    'all', icon=ICONTRAKT,  themeit=THEME2)
    addFile('Recover All Saved Trakt Data', 'restoretrakt', 'all', icon=ICONTRAKT,  themeit=THEME2)
    addFile('Import Trakt Data',            'importtrakt',  'all', icon=ICONTRAKT,  themeit=THEME2)
    addFile('Clear All Saved Trakt Data',   'cleartrakt',   'all', icon=ICONTRAKT,  themeit=THEME2)
...
def realMenu():
    ...
    addFile('[I]https://real-debrid.com is a PAID Service[/I]', '', icon=ICONREAL, themeit=THEME2)
    addFile('Save Real Debrid Data: %s' % real, 'togglesetting', 'keepdebrid', icon=ICONREAL, themeit=THEME2)
    if KEEPREAL == 'true': addFile('Last Save: %s' % str(last), '', icon=ICONREAL, themeit=THEME2)
    ...
    for debrid in debridit.ORDER:
        ...
        else:                        addFile('[COLOR white]Addon Data: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
        if user == "":
    ...
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME6)
    addFile('Save All Real Debrid Data',          'savedebrid',    'all', icon=ICONREAL,  themeit=THEME2)
    addFile('Recover All Saved Real Debrid Data', 'restoredebrid', 'all', icon=ICONREAL,  themeit=THEME2)
    addFile('Import Real Debrid Data',            'importdebrid',  'all', icon=ICONREAL,  themeit=THEME2)
    addFile('Clear All Saved Real Debrid Data',   'cleardebrid',   'all', icon=ICONREAL,  themeit=THEME2)
...
def loginMenu():
    ...
    addFile('[I]Personal API keys provide quicker API responses[/I]', '', icon=ICONLOGIN, themeit=THEME2)
    addFile('Save Login Data: %s' % login, 'togglesetting', 'keeplogin', icon=ICONLOGIN, themeit=THEME2)
    if KEEPLOGIN == 'true': addFile('Last Save: %s' % str(last), '', icon=ICONLOGIN, themeit=THEME2)
    ...
    for login in loginit.ORDER:
        ...
        else:                        addFile('[COLOR white]Addon Data: %s[/COLOR]' % auser,'authlogin', login, icon=icon, fanart=fanart, menu=menu)
        if user == "":
    ...
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME6)
    addFile('Save All Login Data',          'savelogin',    'all', icon=ICONLOGIN,  themeit=THEME2)
    addFile('Recover All Saved Login Data', 'restorelogin', 'all', icon=ICONLOGIN,  themeit=THEME2)
    addFile('Import Login Data',            'importlogin',  'all', icon=ICONLOGIN,  themeit=THEME2)
    addFile('Clear All Saved Login Data',   'clearlogin',   'all', icon=ICONLOGIN,  themeit=THEME2)
...
def favsMenu():
    ...
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME6)
    addFile('Save Favourites: %s' % fav, 'togglesetting', 'keepfavourites', icon=ICONTRAKT, themeit=THEME2)
    if KEEPFAVS == 'true': addFile('Last Save: %s' % str(last), '', icon=ICONTRAKT, themeit=THEME2)
    if HIDESPACERS == 'No': addFile(wiz.sep('Backs up a copy'), '', themeit=THEME6)
    addFile('Save Favourites',      'savefav',    icon=ICONTRAKT,  themeit=THEME2)
    addFile('Recover Favourites',   'restorefav', icon=ICONTRAKT,  themeit=THEME2)
    addFile('Clear Favourite Backup', 'clearfav', icon=ICONTRAKT,  themeit=THEME2)

    setView('files', 'viewType')

def net_tools(view=None):
    addDir ('Speed Tester' ,'speedtestM', icon=ICONAPK, themeit=THEME1)
    addDir ('View IP Address & MAC Address',        'viewIP',   icon=ICONMAINT, themeit=THEME6)

    setView('files', 'viewType')

def viewIP():
    mac,inter_ip,ip,city,state,country,isp = wiz.net_info()
    addFile('[COLOR %s]Mac:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, mac), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Internal IP: [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, inter_ip), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]External IP:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, ip), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]City:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, city), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]State:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, state), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Country:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, country), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]ISP:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, isp), '', icon=ICONMAINT, themeit=THEME6)

    setView('files', 'viewType')

def speedTest():
    addFile('Run Speed Test',            'runspeedtest',      icon=ICONMAINT, themeit=THEME6)
    if os.path.exists(SPEEDTESTFOLD):
        ...
        if len(speedimg) > 0:
            addFile('Clear Results',          'clearspeedtest', icon=ICONMAINT, themeit=THEME6)
            addFile(wiz.sep('Previous Runs'), '', icon=ICONMAINT, themeit=THEME6)
            for item in speedimg:
                ...
                addFile('[B]%s[/B]: [I]Ran %s[/I]' % (img, created), 'viewspeedtest', img, icon=ICONMAINT, themeit=THEME6)
...
def systemInfo():
    ...
    addFile('[COLOR %s]Name:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[0]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Version:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[1]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Platform:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, wiz.platform().title()), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]CPU Usage:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[2]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Screen Mode:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[3]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[B]Uptime:[/B]', '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Current Uptime:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[6]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Total Uptime:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[7]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[B]Local Storage:[/B]', '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Used Storage:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, storage_free), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Free Storage:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, storage_used), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Total Storage:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, storage_total), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[B]Ram Usage:[/B]', '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Used Memory:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, ram_free), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Free Memory:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, ram_used), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Total Memory:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, ram_total), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[B]Network:[/B]', '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Local IP:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[4]), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]External IP:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, exter_ip), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Provider:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, provider), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Location:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, location), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]MacAddress:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, data[5]), '', icon=ICONMAINT, themeit=THEME6)
    totalcount = len(picture) + len(music) + len(video) + len(programs) + len(scripts) + len(skins) + len(repos) 
    addFile('[B]Addons([COLOR %s]%s[/COLOR]):[/B]' % (COLOR5, totalcount), '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Video Addons:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(video))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Program Addons:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(programs))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Music Addons:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(music))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Picture Addons:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(picture))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Repositories:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(repos))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Skins:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(skins))), '', icon=ICONMAINT, themeit=THEME6)
    addFile('[COLOR %s]Scripts/Modules:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR3, COLOR2, str(len(scripts))), '', icon=ICONMAINT, themeit=THEME6)
...
def toggleCache(state):
    cachelist = ['includevideo', 'includeall', 'includehtpctv', 'includeexegesis', 'includescrubsv2', 'includevenom', 'includejudaea', 'includeseren', 'includeexodusredux']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include htpcTV', 'Include Exegesis', 'Include Scrubs v2', 'Include Venom', 'Include Judaea', 'Include Seren', 'Include Exodus Redux']
...
elif mode=='speedtest'      : speedtestMenu()
...
elif mode=='developer'    : developerMenu()
